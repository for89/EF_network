...
library(blavaan)
options(future.globals.maxSize = 4 * 1024^3)  # raise limit to 4 GiB
...
stan_control <- list(
  adapt_delta   = 0.95,  # default is 0.8; higher = fewer divergences
  max_treedepth = 12     # default is 10
)
...
fit_config_b <- bcfa(
  ...
  target      = "stan",
  n.chains    = 4,
  burnin      = 6000,    # up from 4000
  sample      = 5000,
  seed        = 42,
  save.lvs    = FALSE,
  control     = list(
    adapt_delta   = 0.99,  # up from 0.95 — very aggressive
    max_treedepth = 15     # up from 12
  )
)
...
# Convert ELPD differences to LOOIC scale for reporting
# (LOOIC = -2 * ELPD, so LOOIC_diff = -2 * elpd_diff)
invariance_results <- tibble(
  comparison        = c("Configural vs Metric", "Metric vs Scalar"),
  elpd_diff         = c(-301.2, -351.8),
  se_diff           = c(9.2, 15.8),
  looic_diff        = c(-301.2, -351.8) * -2,
  ratio             = abs(c(-301.2, -351.8) / c(9.2, 15.8))
)
invariance_results
...
# Zero-order (raw, unadjusted) correlations within each cohort×wave - shows the "before" picture that motivates partial correlations
...
wave), global strength (Δ), and expected influence (Δ). We evaluated whether changes exceeded a region of practical equivalence (ROPE; |Δr| > 0.05) to distinguish meaningful developmental change from negligible fluctuation.

We found little evidence for meaningful change in EF network summaries across adjacent waves within either cohort (see @tbl-rq2-gt-all). Posterior 95% CrIs for all Δs included 0, and probabilities that edge changes exceeded the ROPE (|Δr| > 0.05) were modest (≈0.3–0.6), indicating substantial uncertainty and no clear support for meaningful change.
