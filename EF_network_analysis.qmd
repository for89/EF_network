...
Children were shown stickers placed on a cartoon ant, which disappeared after a brief delay, and were required to tap the locations where the stickers appeared across an increasing set size (1–8 stickers), with three trials at each level [@howard2017early; @case2013intellectual]. Testing continued until a child failed all three trials at the same level. Working memory capacity was indexed by a point score: one point per consecutive level with at least two correct trials, plus 1/3 point per additional correct trial [@howard2017early].

### Inhibition (Go/No-Go)
Children tapped the screen for go trials (catch a fish; 80% of trials) and withheld responses on no-go trials (avoid a shark; 20% of trials) across 75 trials in three blocks [@howard2017early; @wiebe2012separating]. The high proportion of go trials generated a prepotent response tendency that children were required to overcome on no-go trials. Inhibition was indexed by an impulse control score: the product of go and no-go proportional accuracy.

### Cognitive Flexibility (Card Sorting)
Children sorted stimuli (e.g., blue rabbits, red boats) first by one dimension (e.g., color) before switching to an alternate dimension (e.g., shape); children meeting performance criteria proceeded to a mixed border phase requiring flexible switching between both rules [@howard2017early; @zelazo2006dimensional]. Cognitive flexibility was scored as switch accuracy across post-switch and border phases (range 0–12). A substantial floor effect was observed in the Australian sample at T1 (75/232 children, 32.3%), reflecting persistence on the initial sorting rule consistent with classic DCCS findings [@zelazo2006dimensional].

### Socioeconomic Status

### Income
Caregivers reported gross annual household income in both cohorts. In Australia, bands were aligned with Child Care Subsidy eligibility thresholds: low (AUD $0–$45,144), middle (AUD $45,145–$156,914), or high (>AUD $156,915). In South Africa, income was reported using seven ordered categories adapted from the National Income Dynamics Survey [@nids2017], collapsed into low (categories 1–2), middle (3–5), and high (6–7) to ensure adequate cell sizes.
...
## Software
All analyses were conducted in R (version 4.5.0). Network models were fitted using BGGM (version 2.1.5) and visualizations used ggplot2 (version 4.0.0).
...
A substantial floor effect in Australian T1 cognitive flexibility (32.3% scoring zero; see Appendix @fig-ef-boxplots-by-wave) risks artificial correlations through range restriction and biased partial correlation estimates. We therefore report cross-cohort comparisons on both the full sample and a sensitivity analysis excluding zero-scorers. If results converge, this suggests floor effects do not distort the comparisons; if they diverge, cohort differences may partly reflect measurement artefacts.
