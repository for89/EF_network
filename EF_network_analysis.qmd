...
abstract: |
Executive functions (EFs) are foundational cognitive abilities that develop rapidly during early childhood. Although latent variable approaches have characterized the unity and diversity of EF structure extensively, less is known about the conditional associations among EF components–how these associations change as children develop, and whether they vary across sociocultural contexts. The present study applied Bayesian Gaussian Graphical Models to map age-adjusted EF networks across two longitudinal preschool cohorts from Australia (N = 232; M age = 4.43 years; three waves) and South Africa (N = 217; M age = 4.70 years; two waves). Results suggest moderate and selective coupling among EF components, consistent with partial unity: the inhibition–working memory edge was the most reliable connection across cohorts and waves. Network structure differed between cohorts, with working memory emerging as the most central node in Australia and inhibitory control playing a comparably central role in South Africa — a difference that may reflect the influence of sociocultural and environmental context on EF organization. Despite substantial gains in EF performance across waves, network structure remained stable over time, suggesting a dissociation between the level of executive abilities and the relational architecture organizing them. Exploratory moderation analyses indicated that income-related differences in specific edges emerged at T2 in both cohorts, though these findings require replication in larger samples. Together, these results advance a network perspective on early EF development that complements latent variable approaches, highlighting that the pattern of coupling among EF components — not just children's performance levels — may be sensitive to the environments in which development occurs.
...
We tested measurement invariance using multi-group Bayesian confirmatory factor analysis (bCFA), estimated via Markov chain Monte Carlo sampling in Stan [@merkle2018blavaan; @merkle2021efficient]. Because cognitive flexibility exhibited pronounced floor effects in Australian children at T1 (32.5% scoring zero), and to accommodate heterogeneous measurement scales across cohorts (cognitive flexibility: 0–12 switch accuracy; working memory: 0–4.33 scaled score; inhibition: 0–1 proportion), we treated all three EF indicators as ordinal variables, segmenting them into tertiles (Low/Medium/High) based on pooled distributions to ensure adequate category representation. We fit a one-factor model across four groups simultaneously (AU_T1, AU_T2, SA_T1, SA_T2), testing invariance in a sequence of increasingly constrained models: configural (same factor structure across groups), metric (factor loadings additionally constrained to equality), and scalar (thresholds additionally constrained to equality). We compared model fit using the leave-one-out information criterion (LOO-IC). See Appendix A for further detail.
...
@tbl-render-table1 below presents sample characteristics (age and sex), descriptive statistics for EF scores, and zero-order correlations between EF components by cohort and timepoint. EF scores increased with age across waves (see Appendix @fig-test-re-test-plot and @fig-age-ef-wave), with age-EF correlations ranging from r = `r round(age_cor_min_row$r_age, 2)` (`r ef_label_lookup[as.character(age_cor_min_row$variable)]` in `r country_label_lookup[as.character(age_cor_min_row$country)]` at `r age_cor_min_row$timepoint`) to r = `r round(age_cor_max_row$r_age, 2)` (`r ef_label_lookup[as.character(age_cor_max_row$variable)]` in `r country_label_lookup[as.character(age_cor_max_row$country)]` at `r age_cor_max_row$timepoint`). These negligible to moderate correlations motivated the network approach: partial correlations reveal conditional dependencies after controlling for shared variance and age-related trends.
...
png(net_png, width = 700, height = 550, res = 120)
...
# save forest plot panels to png
forest_png <- "fig-rq1-forest.png"
ggsave(forest_png, p_forest, width = 7, height = 6, dpi = 120)
...
png(final_png, width = 1400, height = 600, res = 120)
grid.newpage()
...
#| fig-cap: "**Global strength and expected influence over time by cohort.** Left panel: points show posterior median; whiskers show 95% credible intervals. Global strength is the sum of absolute partial correlations over unique edges. Right panel: points show posterior median; whiskers show 95% credible intervals. N per wave is shown beneath the zero line within each panel."
#| fig-width: 14
#| fig-height: 6
#| include: true
...
# question form Steven: I imagine this will be taken up in Discussion, but I am wondering how this articulates (or not) with the finding profiled in the Intro that inhibition tends to get subsumed within the central EF factor with additional WM and CF components? At least in AU, does this point to centrality of WM? Or at least the WM-IC (activation/inhibition) coupling?
...
To evaluate cross-cohort comparability of EF indicators, we conducted multi-group Bayesian confirmatory factor analysis (bCFA) across four cohort×wave groups (AU_T1, AU_T2, SA_T1, SA_T2), testing invariance simultaneously across countries and developmental timepoints using Markov chain Monte Carlo estimation via Stan. In ordinal CFA, each observed categorical indicator reflects an underlying continuous latent response variable partitioned by thresholds. Factor loadings capture the strength of association between the latent EF construct and each indicator, while thresholds capture the distributional placement of categories—allowing floor effect severity (e.g., 32.5% of Australian children at T1 scoring zero on cognitive flexibility) to be represented through threshold parameters without distorting factor loadings.
