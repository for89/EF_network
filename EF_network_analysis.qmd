...
@fig-rq1-network-graphs visualizes the network structure. The spatial layout is held constant across panels to facilitate comparison. As described above, working memory consistently occupied a well-connected position, with edges to both inhibition and cognitive flexibility appearing in all three Australian networks and both South African networks. The cognitive flexibility–inhibition edge was absent in Australia at T1, only emerging at T2 when the relationship strengthened sufficiently, consistent with the pattern of increasing coupling seen in the edge estimates.
...
::: {#fig-rq1-combined layout-ncol=2}
```{r}
#| label: fig-rq1-edge-forest
#| fig-cap: "**Within-wave EF coupling (posterior partial correlations).** Points show posterior medians; bars show 95% credible intervals. Solid line = 0; dashed lines = ROPE (±0.05)."
#| include: true
#| fig.width: 8
#| fig.height: 6
...
#| fig-cap: "**EF network structure by cohort and wave.** Nodes positioned consistently across panels; edge thickness reflects partial correlation strength; edges with P(|r|>0.05) < 0.75 are omitted for clarity."
#| include: true
#| out.width: "100%"
#| fig.width: 8
#| fig.height: 6
...
par(mfrow = c(1, 1))
```
EF network structure and within-wave coupling estimates.
:::
...
::: {#fig-rq1-strength-ei layout-ncol=2}
```{r}
...
Global strength and expected influence over time by cohort.
:::
...
#| fig-height: 5
#| fig-pos: 'H'
#| fig-cap: "**Mean EF scores over time (children with repeated EF data).**  
...
One interpretation of this pattern invokes the role of environmental adversity in shaping executive development. The South African sample was drawn from low-income communities in Cape Town characterized by informal housing, poverty, and exposure to community violence [@cook2024risk]. A growing theoretical and empirical literature suggests that environments characterized by chronic stress and threat can selectively promote the development of reactive, threat-focused cognitive processes — including inhibitory control — potentially at the expense of other executive capacities [@howard2020challenging; @blair2016poverty; @sheridan2014dimensions]. Within this framework, the more prominent role of inhibition in the South African EF network may reflect an environment in which inhibitory control is particularly salient and practiced, resulting in stronger coupling between inhibitory processes and cognitive flexibility in early childhood. This interpretation is speculative and cross-sectional with respect to the cohort comparison; nevertheless, it suggests that the topology of the EF network may be a more sensitive index of environmental influence than global connectivity measures.

## Socioeconomic Moderation of EF Network Structure

Exploratory moderation analyses identified two edges that differed meaningfully across income groups, though these should be interpreted cautiously given small income-group sample sizes at later waves and the inflated risk of false positives associated with multiple comparisons across edges. In Australia at T2, higher household income was associated with a weaker cognitive flexibility–working memory edge relative to middle-income families. The direction of this moderation effect may reflect greater differentiation of EF components in higher-SES environments. In EF network development, a weaker edge between two components does not necessarily indicate poorer functioning; it may instead reflect reduced reliance on one ability to engage another, consistent with more specialized and independent EF processes. This interpretation aligns with the broader principle that EF differentiation, rather than integration, may characterize more mature networks [@karr2022unity], though the application of network modelling to EF cognitive data is sufficiently nascent that strong interpretive claims about the meaning of edge strength differences remain premature. 

In South Africa at T2, higher household income was associated with stronger inhibition–working memory coupling. This is  consistent with a resource-dependent co-development account: greater economic resources may support the sustained practice and scaffolding of both inhibitory control and working memory, resulting in stronger functional integration of these components [@rosen2020cognitive]. The directional divergence between Australian and South African income moderation effects — where higher income predicted weaker cognitive flexibility–working memory coupling in Australia but stronger inhibition–working memory coupling in South Africa — may reflect the differing baseline conditions of the two samples. In a low-income sample such as South Africa, income differences may primarily map onto resource availability and basic cognitive scaffolding; in a higher-income sample like Australia, income variation may more strongly relate to differentiated stimulation quality. These interpretations are necessarily speculative and the moderation effects require replication in larger samples with more granular SES measures before strong conclusions can be drawn.
...
#| fig-height: 5
#| fig-pos: 'H'
#| fig-cap: "**Individual EF trajectories over time (children with repeated EF data).**  
