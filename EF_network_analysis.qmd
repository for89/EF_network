...
# consistency check: activities reported but "0 caregivers"
...
sa_data <- sa_data %>%
  mutate(
    # optional: turn refuse/don't know into NA for clarity
    across(all_of(asset_vars), ~ ifelse(.x %in% c(3L, 4L), NA_integer_, as.integer(.x))),
    
    # replicate the data dictionary scoring: count "Yes" (1) across 26 items
    household_items_score_repl = rowSums(
      across(all_of(asset_vars), ~ as.integer(.x == 1L)),
      na.rm = TRUE
    ),
    
    # optional diagnostics
    n_assets_answered = rowSums(!is.na(across(all_of(asset_vars))), na.rm = TRUE)
  )
...
sa_ses <- sa_subset_long %>%
  ...
  transmute(
    ...
    # "income derived from NIDS" (assets score)
...
aus_ef_long <- aus_data %>%
  mutate(id = as.character(.data[[id_var]])) %>%
  filter(!is.na(id)) %>%
...
aus_sex_tp <- aus_any_ids %>%
  left_join(aus_sex_child, by = c("country","id"))

# child-level SES + HLE 
aus_ses_child <- aus_data %>%
  mutate(id = as.character(.data[[id_var]])) %>%   # make id consistent
  filter(!is.na(id)) %>%
  distinct(id, .keep_all = TRUE) %>%
  transmute(
    country    = "Australia",
...
# EF-complete per wave 
ef_complete_ids <- ef_all %>%
  dplyr::group_by(country, timepoint, id) %>%
  dplyr::summarise(
    n_nodes = dplyr::n_distinct(variable[!is.na(value)]),
    .groups = "drop"
  ) %>%
  dplyr::filter(n_nodes == length(ef_levels)) %>%
...
# residualize within wave + z-score within cohort×wave×node
ef_net <- ef_all %>%
  dplyr::inner_join(ef_complete_ids, by = c("country","timepoint","id")) %>%
  dplyr::left_join(age_all %>% dplyr::select(country, timepoint, id, age),
                   by = c("country","timepoint","id")) %>%
  dplyr::left_join(sex_all_tp, by = c("country","timepoint","id")) %>%
  dplyr::filter(!is.na(age), !is.na(sex)) %>%  # decision: require both for residualisation
  dplyr::group_by(country, timepoint, variable) %>%
  dplyr::group_modify(~{
    d <- .x
    fit <- stats::lm(value ~ age + sex, data = d)
    d$resid <- stats::residuals(fit)
    d
  }) %>%
  dplyr::ungroup() %>%
  dplyr::group_by(country, timepoint, variable) %>%
  dplyr::mutate(node = z_within(resid)) %>%
  dplyr::ungroup()

# wide, ready for BGGM per cohort×wave
ef_net_wide <- ef_net %>%
  dplyr::select(country, timepoint, id, variable, node) %>%
  tidyr::pivot_wider(names_from = variable, values_from = node) %>%
  dplyr::filter(dplyr::if_all(dplyr::all_of(ef_levels), ~ !is.na(.)))
```
...
fit_one <- function(df) {
  ...
  BGGM::estimate(
    ...
    burnin = bggm_burnin,
    chains = bggm_chains,
    save   = TRUE   # <-- ADD THIS (critical)
  )
}

ggm_fits_step1 <- purrr::map(dat_split, fit_one)

# keep these objects explicit for Step 1C
fit_tbl <- keys %>%
  mutate(fit = ggm_fits_step1)

# n used per cohort × wave in EF-complete network-estimation sample
n_by_ct <- ef_net_wide %>%
  dplyr::group_by(country, timepoint) %>%
  dplyr::summarise(n = sum(if_all(all_of(nodes), ~ !is.na(.))), .groups = "drop")

sapply(ef_net_wide[, nodes], class)
```
...
find_big_pcor <- function(x, p, min_iter = 1000) {
  # Return the first "big" posterior pcor object found (array or matrix), else NULL
...
get_pcor_draws <- function(fit, nodes = nodes, min_iter = 5000) {
  p <- length(nodes)  # <-- ADD THIS LINE (this is what your error is about)

  # 1) search inside fit
  found <- find_big_pcor(fit, p = p, min_iter = min_iter)
  if (!is.null(found)) return(as_iter_pp(found, p))

  # 2) search inside BGGM::pcor(fit)
  pc_try <- try(BGGM::pcor(fit), silent = TRUE)
  if (!inherits(pc_try, "try-error")) {
    found <- find_big_pcor(pc_try, p = p, min_iter = min_iter)
    if (!is.null(found)) return(as_iter_pp(found, p))
  }

  stop("Could not locate a large posterior sample of partial correlations. Inspect with: str(fit, max.level=3); str(BGGM::pcor(fit), max.level=3)")
}
dim(get_pcor_draws(fit_tbl$fit[[1]], nodes))

```

```{r}
#| label: step1c-edge-summary
#| include: false

edge_pairs <- tibble::tribble(
  ~i, ~j, ~edge_id,                 ~edge,
  1, 2, "ef_inhibition__ef_cogflex", "Inhibition – Cognitive flexibility",
  1, 3, "ef_inhibition__ef_workingmem", "Inhibition – Working memory",
  2, 3, "ef_cogflex__ef_workingmem", "Cognitive flexibility – Working memory"
)

summ_edge <- function(draws, rope = 0.10) {
  purrr::pmap_dfr(edge_pairs, function(i, j, edge_id, edge) {
    ...
    tibble::tibble(
      ...
      edge      = edge,
      median    = median(samp),
      crI_low   = unname(quantile(samp, 0.025)),
      crI_high  = unname(quantile(samp, 0.975)),
      p_abs_gt_0.10 = mean(abs(samp) > rope),
      p_abs_lt_0.10 = mean(abs(samp) < rope)
    )
  })
}
...
edges_step1 <- fit_tbl %>%
  mutate(
    draws = purrr::map(fit, ~ get_pcor_draws(.x, nodes)),
    out   = purrr::map(draws, ~ summ_edge(.x, rope = 0.10))
  )

# NEW: sanity check (must be lots of posterior draws)
draw_sizes <- purrr::map_int(edges_step1$draws, ~ dim(.x)[1])
if (any(draw_sizes < 1000)) {
  stop("Posterior draw extraction failed (too few draws). Inspect: str(fit_tbl$fit[[which.min(draw_sizes)]], max.level=3)")
}

edges_step1 <- edges_step1 %>%
  tidyr::unnest(out) %>%
  left_join(n_by_ct, by = c("country","timepoint")) %>%
  arrange(country, timepoint, edge)

edges_step1

edges_step1 %>%
  select(country, timepoint, edge, n, median, crI_low, crI_high, p_abs_gt_0.10, p_abs_lt_0.10) %>%
  arrange(country, timepoint, edge)

# flag meaningful edges
edges_step1 %>%
  mutate(meaningful = p_abs_gt_0.10 > 0.95) %>%
  select(country, timepoint, edge, median, crI_low, crI_high, p_abs_gt_0.10, meaningful)
```
...
Overall, Step 1 indicates stronger and more consistent within-wave EF coupling in Australia—driven primarily by stable inhibition–working memory coupling—whereas South Africa shows weaker and/or less certain within-wave coupling under the preregistered ROPE decision rule.
