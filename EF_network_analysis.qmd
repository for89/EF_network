...
# Create T1 moderators early so they're available for RQ2b and RQ4
...
# We'll compute changes in raw EF scores and test whether changes in one domain
# predict changes in another (coupling of developmental change)
...
# This tests "coupling of developmental change"
...
# Model: Does baseline SES predict EF change?
...
# Test key interactions (focus on theoretically motivated ones)
...
# Focus on key relationships: WM change predicting other changes
...
edge_did_draws_rz <- function(fit_AU_T1, fit_AU_T2, fit_SA_T1, fit_SA_T2,
  ...
  out <- purrr::map(nms, function(k) {
    ...
    # now align AU and SA delta vectors to same length
...
# Unique fits table (as you already do)
fit_tbl_uniq <- fit_tbl %>%
  dplyr::filter(!purrr::map_lgl(fit, is.null)) %>%
  dplyr::arrange(country, timepoint) %>%
  dplyr::group_by(country, timepoint) %>%
...
rq3_wave_edges_rz <- tibble::tibble(wave = c("T1","T2")) %>%
  ...
  dplyr::mutate(
    # canonical edge key (order invariant) matching your other tables
    term = vapply(
...
# ---- DiD (ΔAU - ΔSA), EDGES ONLY: Δr and Δz ----
...
# Combine into one print table (edges only)
...
# Option A: Split by moderator levels and compare networks
...
# Ensure we have the raw EF data
# These should exist from build-ef-age-objects chunk
stopifnot(exists("ef_all"), exists("age_all"))

# Create local copies to be safe
ef_all_raw <- ef_all
age_all_raw <- age_all
...
# Prepare data for T1 and T2 (both cohorts have these)
...
# This tests whether the three EF components form a coherent construct
...
fit_mi_sequence <- function(data, model, wave_label) {
  ...
  # 1. Configural invariance (baseline)
...
# Helper function to interpret MI results
