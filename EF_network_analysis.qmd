---
title: "Modeling the Dynamics of Executive Function Networks: A Longitudinal Bayesian Network Analysis Across Two Countries" 
shorttitle: "Modeling Executive Function Networks"
author:
...
- name: "Philippe Rast"
  ...
  affiliations:
    - ref: ucdavis
- name: "Catherine E. Draper*"
  orcid: 0000-0002-2885-437X
  affiliations:
    - ref: witwatersrand
- name: "Steven J. Howard*" 
...
abstract: "Executive functions (EFs) are foundational cognitive abilities that develop
  rapidly during early childhood. Although latent variable approaches have
  characterized the unity and diversity of EF structure extensively, less is
  known about the conditional associations among EF components—how these
  associations change as children develop, and whether they vary across
  sociocultural contexts. The present study applied Bayesian Gaussian Graphical
  Models to model age-adjusted EF networks across two longitudinal preschool
  cohorts from Australia (N = 232; M age = 4.43 years; three waves) and South
  Africa (N = 217; M age = 4.70 years; two waves). Results suggest moderate and
  selective coupling among EF components, consistent with partial unity: the
  inhibition–working memory edge was the most reliable connection across cohorts
  and waves. Despite substantial gains in EF performance across waves, network
  structure remained stable over time, suggesting a dissociation between the
  level of executive abilities and the relational architecture organizing them.
  Exploratory moderation analyses indicated that income-related differences in
  specific edges emerged at T2 in both cohorts, though these findings require
  replication in larger samples. Together, these results advance a network
  perspective on early EF development that complements latent variable
  approaches, highlighting that the pattern of coupling among EF components,
  and not just children's performance levels, may be sensitive to the
  environments in which development occurs."

keywords: ["Executive Function", "Bayesian network analysis", "dimensionality"] 
word-count: false
project:
type: default
...
Executive functions (EFs) — inhibitory control, working memory, and cognitive flexibility — are higher-order cognitive control processes that enable goal-directed and adaptive behavior [@diamond2013executive]. Early childhood represents a period of particularly rapid growth in these abilities, coinciding with substantial maturation of the neural systems that underpin cognitive control [@blair2016executive; @best2010developmental; @garon2008executive].

## Unity and Diversity of Executive Function

A key question concerns how EF abilities are organized: do inhibitory control, working memory, and cognitive flexibility reflect distinct systems, or a common control mechanism? The influential unity/diversity framework [@miyake2000unity] demonstrated that these components are moderately correlated yet separable — exhibiting both unity (shared variance) and diversity (component-specific variance). Subsequent bifactor refinements showed that inhibition loads primarily on a Common EF factor, while updating- and shifting-specific factors capture residual variance unique to those domains [@friedman2017unity], with the unity component showing high heritability [@friedman2008individual].

## Developmental Differentiation of Executive Function

Empirical research supports a unidimensional representation of EF during the preschool years, with performance across EF tasks best characterized by a single underlying construct [@nelson2016structure; @wiebe2008using; @willoughby2012measurement; @zelazo2016executive]. This is consistent with theoretical perspectives suggesting executive processes are relatively undifferentiated in young children and become progressively more distinct with age [@bardikoff2017differentiation; @lehto2003dimensions; @miyake2000unity]. Longitudinal and network analyses confirm this trajectory, showing EF transitions from one factor in early childhood to increasingly separable components by late childhood and adolescence [@karr2018unity; @brydges2012unitary; @menu2024latent; @reilly2022developmental], though some preschool studies document early emergence of bifactor solutions [@lee2013developmental; @miller2012latent; @usai2014latent].

## Sociocultural Context and Executive Function Organization

A related but largely unexamined question concerns whether the organizational structure of EF — not just its level — varies across sociocultural contexts. Research increasingly demonstrates that cultural practices, socioeconomic conditions, and everyday experiences shape not only EF performance but the processes underlying it: children's ability to delay gratification is context-specific, reflecting culturally acquired waiting habits rather than a general self-control capacity, such that the same children show markedly different profiles depending on whether the reward context aligns with their cultural practices [@yanaoka2022cultures]. More broadly, social contextual factors — including norms, trust, and cultural values — shape whether and how children engage executive functions, raising fundamental questions about what cross-group differences in EF reflect [@doebel2020rethinking; @munakata2021executive]. Cross-cultural studies of EF factor structure suggest that while broad organizational patterns are often preserved across contexts, the strength of associations among components can vary [@schirmbeck2020similarities]. Yet this work has focused on factor-level invariance rather than the conditional dependencies among EF components directly. Whether the relational architecture of EF — which components are most tightly coupled, and how this changes developmentally — is universal or context-sensitive remains unknown. Australia and South Africa represent meaningfully contrasting sociocultural and socioeconomic contexts for examining this question: prior work using the same samples and measures found that the most disadvantaged South African children outperformed middle- and high-SES Australian peers on inhibition and cognitive flexibility [@howard2020challenging], suggesting that context shapes EF in ways that cut across simple SES predictions.

## Neural Architecture of Executive Function

The brain networks supporting EF have been characterized in neuroimaging research with adults. The fronto-parietal network (FPN) and cingulo-opercular network (CON) are the core control networks central to executive processing [@dosenbach2008dual; @niendam2012meta], with a superordinate fronto-cingulo-parietal network consistently engaged across EF domains — providing neural evidence for the unity component observed in behavioral models [@niendam2012meta]. More recent work proposes this shared architecture corresponds to the 'multiple demand' (MD) system — frontal, parietal, and posterior temporal regions active across diverse cognitive demands — as the neural substrate for EF unity [@duncan2024brain]. Alongside this shared architecture, component-specific neural signatures mirror the diversity observed in behavioral factor structures: inhibition, working memory, and shifting tasks produce overlapping activations within common network territories while each recruiting component-specific regions [@assem2024basis; @collette2006exploration; @mckenna2017informing; @rodriguez2022inhibition; @rottschy2012modelling], complementing nested-factor behavioral models wherein a Common EF factor coexists with component-specific factors.

## Developmental Emergence of Executive Networks

During early childhood, neural networks supporting EF are less differentiated than in adults, with weaker frontal-frontal interactions and less refined fronto-parietal connectivity at ages 3–4, and strengthened connections emerging by ages 4–5 [@buss2018changes; @fiske2019neural; @kelsey2025forming]. The core architecture of adult executive networks is established by middle childhood (~8–10 years), after which developmental improvements in EF reflect strengthening of connectivity within established networks rather than reorganization of overall network structure [@engelhardt2019neural]. Across childhood and adolescence, activation patterns show progressive differentiation — younger children recruit prefrontal regions more diffusely, while with age there is increasing specialization within FPN and CON [@crone2006neurocognitive; @rubia2013functional]. This neural trajectory closely parallels the behavioral differentiation of EF components, with developmental specialization occurring through synaptic pruning, myelination, and refinement of functional connectivity [@baum2020development; @tamnes2017development], underscoring that the unity-diversity structure of EF is grounded in the developing architecture of distributed brain networks [@karr2018unity; @li2025structural].

## Methodological Approaches to Studying Executive Organization

Capturing this developmental trajectory in behavioral data requires analytic approaches that can model the structure of EF — not just its level — and track how that structure changes with age. Latent variable methodologies have been central to advancing the unity/diversity framework: confirmatory factor analysis addresses task impurity by extracting shared variance across tasks, enabling comparison of factor structures across age groups while accounting for measurement error [@karr2018unity; @wiebe2008using]. However, latent modelling relies on specific assumptions about how executive abilities are represented, and the factor structure obtained can vary depending on tasks selected, performance indicators, and analytic specification [@miller2012latent; @vandierendonck2021utility]. Importantly, latent models characterize executive organization in terms of shared variance explained by unobserved factors, and do not directly model conditional dependencies among observed executive task variables — a meaningful limitation from a systems perspective, given that executive control reflects coordinated activity within distributed neural networks. Network analytic approaches address this directly — @karr2022unity was among the first to apply Bayesian Gaussian Graphical Models (BGGM) to EF across developmental stages, with subsequent work integrating network and latent approaches [@menu2024latent]. By modeling conditional associations among executive components, a network perspective provides a systems-level complement to latent approaches and enables more direct investigation of developmental change in executive architecture.

## The Present Study 

The present study applies Bayesian Gaussian Graphical Models to examine the organization of EF during early childhood across two longitudinal cohorts from Australia and South Africa, estimating age-adjusted partial correlations among inhibitory control, working memory, and cognitive flexibility. This approach captures direct conditional associations among EF components after accounting for shared relationships and age-related variance, enabling investigation of which components show direct coupling, how these associations change developmentally, and whether network structure varies across sociocultural and socioeconomic contexts. Finally, we exploratorily examine whether SES moderates EF network structure within each cohort — specifically, whether edges are systematically stronger or weaker among children from high versus low household income and home learning environment backgrounds. If socioeconomic conditions shape not just EF level but the conditional dependencies among components, this would suggest that the relational architecture of EF is sensitive to proximal environmental variation within as well as between sociocultural contexts.

Understanding how EF components are organized during early development holds implications for both theory and practice. Network analysis can reveal whether components function as tightly coupled systems or operate more independently, informing intervention design: strong coupling would favor broad general executive control training, while weak coupling would favor targeted domain-specific approaches [@li2025structural; @scerif2025enhancing]. Longitudinal network analysis also offers a means of detecting intervention effects directly — strengthening or reorganization of specific edges following targeted training would provide fine-grained evidence that an intervention has altered conditional relationships among EF components — though interpretation must be grounded in theory, since differentiation accounts predict that adaptive change involves increasing component independence rather than uniformly stronger coupling.

### Research Questions

**Research Question 1 (Primary): EF Network Structure Within Cohorts**
Within each cohort and wave, what is the age-adjusted structure of the EF network — defined by partial correlations among working memory, inhibition, and cognitive flexibility — in terms of overall connectivity and node centrality?
...
#### Australia
The Australian sample was the control group from an intervention study, comprising 232 children recruited from 25 preschools across diverse socioeconomic metropolitan and regional areas of New South Wales (M age = 4.43 years, SD = 0.38; 46.1% girls). Ninety-four percent were retained at T2. A geographically constrained subset (within 1 hour of the lead institution) was followed at T3, yielding a sample of 105 children with no significant demographic differences from those not followed up. For full details on recruitment and sample characteristics, see @howard2020everyday.

#### South Africa
The South African sample was drawn from a longitudinal study recruiting 243 preschool-aged children from four low-income communities in Cape Town characterized by informal housing, poverty, and exposure to violence [@cook2024risk]. The analytic sample comprised children with EF data at each timepoint: 217 at T1 (M age = 4.70 years, SD = 0.60; 48.1% female) and 189 at T2. For full details see @cook2024risk.

## Measures

### Executive Function
EF was assessed using three measures from the Early Years Toolbox (EYT; @howard2017early), a suite of iPad-based assessments with integrated audio instructions ensuring standardized delivery across languages. The EYT has been validated across diverse cultural contexts including South Africa [@cook2019associations; @draper2020understanding] and internationally [@hossain2021international; @munambah202124].

#### Working Memory (Mr. Ant)
Children are shown stickers placed on a cartoon ant, which disappear after a brief delay, and must tap the locations where the stickers appeared across increasing set sizes (1–8 stickers), with three trials at each level [@howard2017early; @case2013intellectual]. Testing continues until a child fails all three trials at the same level. Working memory capacity was indexed by a point score: one point per consecutive level with at least two correct trials, plus 1/3 point per additional correct trial [@howard2017early].

#### Inhibition (Go/No-Go)
Children tap the screen for go trials (catch a fish; 80% of trials) and withhold responses on no-go trials (avoid a shark; 20% of trials) across 75 trials in three blocks [@howard2017early; @wiebe2012separating]. The high proportion of go trials generates a prepotent response tendency that children must overcome on no-go trials. Inhibition was indexed by an impulse control score: the product of go and no-go proportional accuracy.

#### Cognitive Flexibility (Card Sorting)
Children sort stimuli (e.g., blue rabbits, red boats) first by one dimension (e.g., color) then switch to an alternate dimension (e.g., shape); children meeting performance criteria proceed to a mixed border phase requiring flexible switching between both rules [@howard2017early; @zelazo2006dimensional]. Cognitive flexibility was scored as switch accuracy across post-switch and border phases (range 0–12). A substantial floor effect was observed in the Australian sample at T1 (75/232 children, 32.3%), reflecting perseveration on the initial sorting rule consistent with classic DCCS findings [@zelazo2006dimensional].
...
#### Australia
Caregivers reported gross annual household income using three bands aligned with Australia's Child Care Subsidy eligibility thresholds: low (AUD $0–$45,144), middle (AUD $45,145–$156,914), or high (>AUD $156,915).

#### South Africa
Caregivers reported household income using items adapted from the National Income Dynamics Survey [@nids2017], yielding seven ordered categories. To facilitate comparability with the Australian sample and ensure adequate cell sizes, categories were collapsed into three: low (categories 1–2), middle (categories 3–5), and high (categories 6–7).

### The Home Learning Environment

#### Australia
Caregivers reported the frequency of eight home learning activities in the past 7 days (e.g., reading to the child, visiting the library, teaching sports), using a 5-point scale from 0 to 7 days, adapted from the EPPSE study [@melhuish2008effects; @sylva2004effective]. An HLE composite score was calculated by summing responses across all eight activities.

#### South Africa
Caregivers reported how frequently their child engaged in eight enrichment activities in the past 7 days (e.g., reading books, counting, singing songs) using a 3-point scale (never, sometimes, many times), using a measure developed for the South African context [@dawes2020early; @melhuish2010impact]. An HLE composite score was calculated by summing frequencies across all eight activities.

## Statistical Analysis

### Network Estimation
We estimated EF network structure using Bayesian Gaussian Graphical Models (BGGM) [@williams2020bayesian; @williams2020bggm] implemented in the BGGM R package [@BGGM]. Separate models were fit for each cohort×wave combination, estimating conditional associations among the three EF components after controlling for age. We used default priors and ran MCMC sampling for 5,050 iterations (5,000 posterior samples after 50 burn-in iterations).

### Edge Weights
Edge weights are partial correlations between pairs of EF components — that is, the association between two components after controlling for the third and for age. A positive edge indicates that two components covary above and beyond their shared relationships with other components; values closer to 0 indicate weaker associations. For each edge we report the posterior median and 95% credible interval (CrI), which can be directly interpreted as a probability statement about the parameter.

### Meaningfulness Threshold
To distinguish meaningful edges from negligible ones, we adopted a Region of Practical Equivalence (ROPE) approach [@kruschke2018rejecting], defining the ROPE as |r| ≤ 0.05 [@huth2025statistical]. Rather than testing whether an edge differs from exactly zero, this approach asks whether it is large enough to be practically meaningful. For each edge we computed P(|r| > 0.05): the posterior probability that the edge exceeds the ROPE threshold. Values near 1 (e.g., P > 0.90) indicate strong evidence of a meaningful edge; values near 0.50 indicate substantial uncertainty; values near 0 indicate the edge is practically equivalent to zero.

### Global Strength
Global strength summarizes overall network connectivity as the sum of absolute edge weights across all unique pairs, with higher values indicating more strongly coupled EF components. We computed global strength across all posterior samples, yielding a full posterior distribution for this metric. Formally, for a network with *p* nodes, global strength is:
$$
GS = \sum_{i<j} |r_{ij}|,
$$
...
Centrality (Expected Influence)
Expected Influence [@robinaugh2016identifying] quantifies the centrality of each node — that is, how strongly connected it is to other nodes — as the sum of a node's signed edge weights to all other nodes. Nodes with higher expected influence are more central to the network and may play a more important role in the overall system. Unlike older centrality metrics (e.g., betweenness, closeness), expected influence is appropriate for networks estimated from partial correlations and preserves the sign of relationships, providing a more nuanced picture than absolute-value alternatives when edges vary in direction. Formally, for node *i*:
$$
EI_i = \sum_{j \neq i} r_{ij}
$$

### Change and Comparison Analyses
To test for developmental change within cohorts (RQ2), we computed difference scores between adjacent waves (e.g., Δr = r~T2~ − r~T1~) by subtracting posterior samples pairwise, yielding a posterior distribution of change from which we derived 95% CrIs and ROPE probabilities (P(|Δr| > 0.05)).

For cross-cohort comparisons (RQ3), we computed cohort difference scores (e.g., r~AU~ − r~SA~) at matched waves. To account for the bounded nature of correlations, we also report differences on the Fisher z-scale (z = atanh(r)), which transforms correlations to an unbounded scale where differences are more symmetric [@williams2020back].

For difference-in-differences (DiD) contrasts, we tested whether cohorts showed divergent developmental trajectories by computing (Δr~T2-T1~)~AU~ − (Δr~T2-T1~)~SA~.

### Moderation Analyses
To test whether socioeconomic factors moderate network structure (RQ4), we split children within each cohort into High vs. Low groups based on median splits of baseline (T1) income and HLE, then fit separate BGGM models for each group at each wave. This stratified approach allows network structure to differ nonlinearly across SES groups. Moderation effects were computed as High − Low edge differences, with ROPE probabilities indicating strength of evidence (P(|High − Low| > 0.05)).

## Software
All analyses were conducted in R (version 4.5.0). Network models were fitted using BGGM (version 2.1.5) and visualizations used ggplot2 (version 4.0.0). Analysis code is available via GitHub.
...
library(haven)
aus_data <- read_sav(Sys.getenv("DATA_PATH_1"))
colnames(aus_data)
```
...
path <- Sys.getenv("DO_FILE_TXT_PATH")
# system2("open", path)

do_lines <- read_lines(Sys.getenv("DO_FILE_PATH"))
...
sa_data <- read.csv(Sys.getenv("DATA_PATH_2"))

sa_data <- read_csv(
  Sys.getenv("DATA_PATH_2"),
  col_names = FALSE
)
...
tp_sex <- bind_rows(
  ...
  complete(
    nesting(country, timepoint),
    sex = c("Female", "Male", "Other/Unknown", "Missing"),
    fill = list(n = 0)
  ) %>%
  group_by(country, timepoint) %>%
  mutate(pct = 100 * n / sum(n)) %>%
...
make_group_levels <- function(...) {
  bind_rows(...) %>%
    ...
    arrange(match(country, c("Australia", "South Africa")), timepoint) %>%
    pull(group)
}
...
```{r}
#| label: build-cor-raw-table
#| include: false

cor_raw_wide <- ef_all_keep %>%
  mutate(
    country   = country_for_table(country),
    timepoint = as.character(timepoint)
  ) %>%
...
# Age-EF correlations by cohort×wave
...
# Label lookups for readable prose
ef_label_lookup <- c(
  ef_inhibition  = "inhibition",
  ef_cogflex     = "cognitive flexibility",
  ef_workingmem  = "working memory"
...
country_label_lookup <- c(AU = "Australia", SA = "South Africa")

# Min and max correlation rows
age_cor_min_row <- age_ef_cors %>% slice_min(r_age, n = 1)
age_cor_max_row <- age_ef_cors %>% slice_max(r_age, n = 1)

# Helper function (kept for any manual lookups)
get_age_r <- function(ctry, tp, var) {
  val <- age_ef_cors$r_age[
    as.character(age_ef_cors$country)   == ctry &
    as.character(age_ef_cors$timepoint) == tp   &
    as.character(age_ef_cors$variable)  == var
  ]
  if (length(val) == 0) return(paste0("[NO MATCH: ", ctry, "/", tp, "/", var, "]"))
  round(val, 2)
}
```
# Results
...
@tbl-render-table1 presents sample characteristics (age and sex), descriptive statistics for EF scores, and zero-order correlations between EF components by cohort and timepoint. EF scores increased with age across waves (see Appendix @fig-age-ef-wave), with age-EF correlations ranging from r = `r round(age_cor_min_row$r_age, 2)` (`r ef_label_lookup[as.character(age_cor_min_row$variable)]` in `r country_label_lookup[as.character(age_cor_min_row$country)]` at `r age_cor_min_row$timepoint`) to r = `r round(age_cor_max_row$r_age, 2)` (`r ef_label_lookup[as.character(age_cor_max_row$variable)]` in `r country_label_lookup[as.character(age_cor_max_row$country)]` at `r age_cor_max_row$timepoint`), justifying residualization of age prior to network estimation. Zero-order correlations between EF components were small to moderate (r = `r round(cor_summary$min_r, 2)`–`r round(cor_summary$max_r, 2)`; @tbl-render-table1), indicating partially overlapping but distinct constructs. A network approach was therefore used to examine the conditional dependencies between EF components — that is, associations between pairs of measures after accounting for their shared variance with the remaining component.

```{r}
#| label: report-missingness
#| include: false

missingness_ct <- ef_all_keep %>%
...
ef_long <- add_group(tp_ef_desc) %>%
  ...
  transmute(
    ...
    row     = paste0(domain, ", Mean (SD) [n]"),
    group,
    value   = paste0(fmt_mean_sd(mean, sd, digits = 2), " [", n, "]")
  ) %>%
  select(section, row, group, value)
...
miss_long <- missingness_ct %>%
  mutate(
    country   = country_for_table(country),
    timepoint = as.character(timepoint),
    group     = paste(country, timepoint, sep = " — "),
    section   = "Executive function",
    row       = "Missing EF data, n cells (%)",
    value     = paste0(missing_cells, " (", sprintf("%.1f", pct_missing), "%)")
  ) %>%
  dplyr::select(section, row, group, value)

cor_long <- cor_raw_table %>%
...
metrics1_long <- bind_rows(age_long, sex_long, ef_long, miss_long, cor_long) %>%
  mutate(
...
# Capture intended row order before widening
row_order <- metrics1_long %>%
  distinct(section, row) %>%
...
paper_table1 <- metrics1_long %>%
  ...
  select(section, row, group, value) %>%
  pivot_wider(names_from = group, values_from = value) %>%
  left_join(row_order, by = c("section", "row")) %>%
  arrange(row_idx) %>%
  select(-row_idx)

au_cols <- grep("Australia",    names(paper_table1), value = TRUE)
sa_cols <- grep("South Africa", names(paper_table1), value = TRUE)
```

```{r}
# #| results: asis
# #| include: true
# knitr::raw_latex("\\begin{landscape}\n")
```

```{r}
#| label: tbl-render-table1
#| tbl-cap: "Age and Executive Function (EF) by country and timepoint. All summaries are anchored to children with any EF observed at that timepoint (partial EF allowed)."
#| include: true
...
all_data_cols <- c(au_cols, sa_cols)
n_au          <- length(au_cols)
n_sa          <- length(sa_cols)

# Row indices just before each new section starts — these get a thin divider
section_starts <- which(!duplicated(paper_table1$section))
divider_rows   <- section_starts[-1] - 1

# Replace NA with em dash upfront
tbl1 <- paper_table1 %>%
  mutate(across(all_of(all_data_cols),
                ~ ifelse(is.na(.), "\u2014", as.character(.))))

# Bottom header row labels: strip "Country — " prefix, blank out section/row
header_labels <- c(
  section = "",
  row     = "",
  setNames(gsub(".* \u2014 ", "", all_data_cols), all_data_cols)
)

flextable(tbl1, col_keys = c("section", "row", all_data_cols)) %>%

  # 2 level header
  add_header_row(
    values    = c("", "", "Australia", "South Africa"),
    colwidths = c(1L, 1L, n_au, n_sa),
    top       = TRUE
  ) %>%
  set_header_labels(values = header_labels) %>%

  # Merge repeated section values into one cell per group
  merge_v(j = "section", part = "body") %>%
  valign(j = "section", valign = "top", part = "body") %>%

  # Borders (APA-style)
  border_remove() %>%
  hline_top(border = thick, part = "header") %>%
  hline(i = 1, border = thick, part = "header") %>%
  hline_bottom(border = thin,  part = "header") %>% 
  hline(i = divider_rows, border = thin,  part = "body") %>%  
  hline_bottom(border = thick, part = "body") %>%         

  # Alignment
  align(align = "center", part = "header") %>%
  align(align = "center", part = "body") %>%
  align(j = c("section", "row"), align = "left", part = "body") %>%
  
   # Text formatting
  bold(i = 1, part = "header") %>%                        
  italic(j = "section", part = "body") %>%                
  padding(j = "row", padding.left = 14, part = "body") %>% 
  ...
  # Footer
  add_footer_lines(
    "Note. Missing EF data counts reflect missing cells across all three EF measures."
  ) %>%
...
summ_draws <- function(draws, rope = 0.05) {
...
# rows per cohort×wave (3 edges + global strength)
...
Prior to the primary analyses, we evaluated the cross-cohort comparability of EF indicators using multi-group Bayesian confirmatory factor analysis across all four cohort×wave groups (AU_T1, AU_T2, SA_T1, SA_T2). A sequence of increasingly constrained models was compared using the leave-one-out information criterion (LOOIC). Both metric invariance (equal factor loadings) and scalar invariance (equal thresholds) were supported, with each additional set of constraints improving predictive fit by a margin well exceeding its standard error. Full scalar invariance supports the comparison of structures and latent performance levels across cohorts [@vandenberg2000review; @putnick2016measurement]. Full details of the invariance testing procedure and results are reported in Appendix B.

### Research Question 1 (Primary): EF Network Structure Within Cohorts

### Edges

Within-wave age-adjusted partial correlations are summarized in @tbl-rq1-gt-all and visualized in @fig-rq1-combined below. In Australia, edges involving working memory showed the clearest evidence of coupling across waves. The inhibition–working memory edge was consistently positive (T1: `r edge_report("Australia", "T1", "Inhibition – Working memory")`; T2: `r edge_report("Australia", "T2", "Inhibition – Working memory")`; T3: `r edge_report("Australia", "T3", "Inhibition – Working memory")`) and reliably exceeded the meaningfulness threshold (T1: `r edge_meaningful("Australia", "T1", "Inhibition – Working memory")`; T2: `r edge_meaningful("Australia", "T2", "Inhibition – Working memory")`; T3: `r edge_meaningful("Australia", "T3", "Inhibition – Working memory")`). The cognitive flexibility–working memory edge was also positive across waves (T1: `r edge_report("Australia", "T1", "Cognitive flexibility – Working memory")`; T2: `r edge_report("Australia", "T2", "Cognitive flexibility – Working memory")`; T3: `r edge_report("Australia", "T3", "Cognitive flexibility – Working memory")`) and exceeded the meaningfulness threshold though with declining certainty across waves (meaningfulness: T1: `r edge_meaningful("Australia", "T1", "Cognitive flexibility – Working memory")`; T2: `r edge_meaningful("Australia", "T2", "Cognitive flexibility – Working memory")`; T3: `r edge_meaningful("Australia", "T3", "Cognitive flexibility – Working memory")`). The cognitive flexibility–inhibition edge was weaker and more wave-dependent (T1: `r edge_report("Australia", "T1", "Cognitive flexibility – Inhibition")`; T2: `r edge_report("Australia", "T2", "Cognitive flexibility – Inhibition")`; T3: `r edge_report("Australia", "T3", "Cognitive flexibility – Inhibition")`), with limited evidence of exceeding the meaningfulness threshold at T1 (T1: `r edge_meaningful("Australia", "T1", "Cognitive flexibility – Inhibition")`; and more consistent but still moderate evidence at later waves -- T2: `r edge_meaningful("Australia", "T2", "Cognitive flexibility – Inhibition")`; T3: `r edge_meaningful("Australia", "T3", "Cognitive flexibility – Inhibition")`).

In South Africa, the inhibition–working memory edge remained positive across waves (T1: `r edge_report("South Africa", "T1", "Inhibition – Working memory")`; T2: `r edge_report("South Africa", "T2", "Inhibition – Working memory")`) though it exceeded the meaningfulness threshold with moderate rather than strong certainty (T1: `r edge_meaningful("South Africa", "T1", "Inhibition – Working memory")`; T2: `r edge_meaningful("South Africa", "T2", "Inhibition – Working memory")`). The cognitive flexibility–inhibition edge was present at T1 (`r edge_report("South Africa", "T1", "Cognitive flexibility – Inhibition")`) but attenuated markedly by T2 (`r edge_report("South Africa", "T2", "Cognitive flexibility – Inhibition")`) 
providing little evidence of a reliable association at the later wave (T1: `r edge_meaningful("South Africa", "T1", "Cognitive flexibility – Inhibition")`; T2: `r edge_meaningful("South Africa", "T2", "Cognitive flexibility – Inhibition")`). The cognitive flexibility–working memory edge was weaker and uncertain across waves (T1: `r edge_report("South Africa", "T1", "Cognitive flexibility – Working memory")`; T2: `r edge_report("South Africa", "T2", "Cognitive flexibility – Working memory")`).

@fig-rq1-combined visualizes the network structure, with spatial layout held constant across panels to facilitate comparison. Working memory consistently occupied a well-connected position, with edges to both inhibition and cognitive flexibility present in all Australian and South African networks. The cognitive flexibility–inhibition edge was absent in Australia at T1, emerging at T2 as the relationship strengthened, consistent with the increasing coupling seen in the edge estimates.
...
rq1_table_all <- dplyr::bind_rows(
  ...
  mutate(across(c(estimate, rope_p),
                ~ ifelse(is.na(.) | . == "", "\u2014", as.character(.))))

col_keys <- c("panel", "term", "country", "timepoint", "n", "pct_missing", "estimate", "rope_p")

section_starts <- which(!duplicated(rq1_table_all$panel))
divider_rows   <- section_starts[-1] - 1

flextable(rq1_table_all, col_keys = col_keys) %>%
  set_header_labels(
    panel       = "",
    term        = "Measure",
    country     = "Cohort",
  ...
  merge_v(j = "panel", part = "body") %>%
  valign(j = "panel", valign = "top", part = "body") %>%
  border_remove() %>%
  hline_top(border = thick, part = "header") %>%
  hline_bottom(border = thin,  part = "header") %>%
  hline(i = divider_rows, border = thin,  part = "body") %>%
  ...
  align(align = "center", part = "header") %>%
  align(align = "center", part = "body") %>%
  align(j = c("panel", "term"), align = "left", part = "body") %>%
  italic(j = "panel", part = "body") %>%
  padding(j = "term", padding.left = 14, part = "body") %>%
  fontsize(size = 10, part = "all") %>%
  set_table_properties(layout = "autofit", width = 1) %>%
  add_footer_lines(
    "Note. CrI = credible interval; EI = expected influence; GS = global strength. ROPE threshold = 0.05."
  ) %>%
  italic(i = 1, part = "footer")
```
...
#| warning: false
#| message: false
#| fig-cap: "EF network structure and within-wave coupling estimates. (A) EF network structure by cohort and wave. Nodes positioned consistently across panels; edge thickness reflects partial correlation strength; edges with P(|r|>0.05) < 0.75 are omitted for clarity. (B) Within-wave EF coupling (posterior partial correlations). Points show posterior medians; bars show 95% credible intervals. Solid line = 0; dashed lines = ROPE (±0.05)."
...
get_edge_inclusion <- function(fit, nodes, rope = 0.05) {
  A <- fit$post_samp$pcors
  p <- length(nodes)
...
plot_ef_network <- function(fit, title, threshold = 0.75) {
  ...
  inclusion <- get_edge_inclusion(fit, nodes, rope = 0.05)
...
edges_df2 <- edge_tbl %>%
  ...
  dplyr::mutate(
    ...
    edge_short = factor(edge_short,
                        levels = c("Inhib\u2013WM", "CogFlex\u2013WM", "CogFlex\u2013Inhib"))
  ) %>%
  dplyr::filter(!(timepoint == "T3" & country == "SA")) %>%
  dplyr::mutate(
    facet_combo = paste(timepoint, country, sep = " "),
    facet_combo = factor(facet_combo,
                         levels = c("T1 AU", "T1 SA", "T2 AU", "T2 SA", "T3 AU"))
  )

p_forest <- ggplot(edges_df2, aes(x = q50, y = edge_short, colour = edge_short)) +
  geom_rect(
  ...
  guides(colour = "none") +
  labs(x = "Partial correlation (posterior median, 95% CrI)", y = NULL) +
...
# Save intermediate PNGs at 300 dpi
net_png    <- "Figure_1_networks.png"
forest_png <- "Figure_1_forest.png"
final_png  <- "Figure_1.png"

png(net_png, width = 1950, height = 1650, res = 300)
...
ggsave(forest_png, p_forest,
       width = 6.5, height = 5.5, units = "in", dpi = 300)

# Combine side by side at 300 dpi
img_net    <- png::readPNG(net_png)
img_forest <- png::readPNG(forest_png)

png(final_png, width = 4200, height = 1650, res = 300)
grid.newpage()
pushViewport(viewport(layout = grid.layout(
  1, 2, widths = unit(c(0.45, 0.55), "npc"))))

# Panel A
pushViewport(viewport(layout.pos.col = 1))
pushViewport(viewport(layout = grid.layout(
  2, 1, heights = unit(c(0.04, 0.96), "npc"))))
pushViewport(viewport(layout.pos.row = 1))
grid.text("(A)", x = 0.05, y = 0.5, just = "left",
...
pushViewport(viewport(layout = grid.layout(
  2, 1, heights = unit(c(0.04, 0.96), "npc"))))
pushViewport(viewport(layout.pos.row = 1))
grid.text("(B)", x = 0.05, y = 0.5, just = "left",
          gp = gpar(fontsize = 12, fontface = "bold"))
popViewport()
pushViewport(viewport(layout.pos.row = 2))
grid.raster(img_forest)
popViewport()
popViewport()
popViewport()
...
knitr::include_graphics(final_png)
```
...
Global strength estimates (sum of absolute partial correlations over unique edges) are reported in @tbl-rq1-gt-all and visualized in @fig-rq1-strength-ei. Credible intervals overlapped substantially across timepoints, indicating no reliable evidence of change in global strength over time, despite variation in point estimates (e.g., T1 to T2 in the SA cohort).

### Centrality (Expected influence)
Expected influence estimates (signed sum of incident partial correlations) are also summarized in @tbl-rq1-gt-all and visualized by node in @fig-rq1-strength-ei. Expected influence was consistently positive across cohorts, with working memory showing the highest centrality and inhibition also relatively high, particularly from T2 onward in Australia. However, credible intervals overlapped substantially across waves, meaning apparent shifts in point estimates cannot be interpreted as reliable longitudinal change. Results thus support a stable relative ordering of node centrality, with working memory (and to a lesser extent inhibition) contributing most strongly to within-wave coupling.
\clearpage
...
```{r}
#| label: fig-rq1-strength-ei
#| fig-cap: "**Global strength and expected influence over time by cohort.** (A) Global strength: points show posterior median; whiskers show 95% credible intervals. Global strength is the sum of absolute partial correlations over unique edges. (B) Expected influence: points show posterior median; whiskers show 95% credible intervals. N per wave is shown beneath the zero line within each panel."
#| fig-width: 14
#| fig-height: 6
#| include: true
...
p_gs <- ggplot(gs_df, aes(x = timepoint, y = q50, group = country)) +
  geom_line(linewidth = 0.7) +
  geom_errorbar(aes(ymin = q025, ymax = q975), width = 0.08, linewidth = 0.7) +
  geom_point(size = 2.0) +
  facet_grid(. ~ country) +
  labs(x = "Wave", y = "Global strength (posterior median, 95% CrI)") +
  theme_bw(base_size = 11)
...
p_ei <- ggplot(rq1_ei, aes(x = timepoint, y = q50, group = 1)) +
  ...
  labs(x = "Wave", y = "Expected influence (posterior median, 95% CrI)") +
  theme_bw(base_size = 11) +
  theme(
    strip.placement    = "outside",
...
# Save high-res PNG for submission
ggsave("Figure_2.png", p_combined,
...
For each wave transition, we computed posterior distributions of change in edge weights (Δr = later wave − earlier wave), global strength (Δ), and expected influence (Δ). We evaluated whether changes exceeded a region of practical equivalence (ROPE; |Δr| > 0.05) to distinguish meaningful developmental change from negligible fluctuation. We found little evidence for meaningful change in EF network summaries across adjacent waves within either cohort (see @tbl-rq2-gt-all). Posterior 95% CrIs for all Δs included 0, and probabilities that edge changes exceeded the ROPE (|Δr| > 0.05) were modest (≈0.6–0.8), indicating substantial uncertainty and no clear support for meaningful change.

Despite the absence of meaningful change in EF network structure across adjacent waves, mean EF performance increased clearly across the same period (see @fig-mean-trend below and Appendix @fig-spaghetti-plot for individualized trajectories). Children in both cohorts showed reliable improvement in raw EF scores across waves — consistent with the well-documented rapid growth of executive abilities during the preschool years [@best2010developmental; @diamond2013executive] — yet the conditional associations among EF components remained largely stable in both magnitude and pattern.
```{r}
#| label: tbl-rq2-gt-all
#| tbl-cap: "Change in EF network summaries across adjacent waves within cohort. Cells show posterior median [95% CrI] for Δ (later − earlier). For edges, cells also show $P(|\\Delta r| > 0.05)$."
#| include: true

rq2_table_print <- rq2_table_all %>%
  dplyr::mutate(
    term  = dplyr::recode(term, !!!node_to_label),
    across(c(estimate, rope_p),
           ~ ifelse(is.na(.) | . == "", "\u2014", as.character(.)))
  )

col_keys <- c("panel", "term", "country", "contrast", "estimate", "rope_p")

section_starts <- which(!duplicated(rq2_table_print$panel))
divider_rows   <- section_starts[-1] - 1

flextable(rq2_table_print, col_keys = col_keys) %>%
  set_header_labels(
    panel    = "",
    term     = "Measure",
    country  = "Cohort",
  ...
  set_table_properties(layout = "autofit", width = 1) %>%
  add_footer_lines(
    "Note. CrI = credible interval; \u0394 = change (later \u2212 earlier wave). ROPE threshold = 0.05."
  ) %>%
  italic(i = 1, part = "footer")
```
...
#|   *Note. "EF observed" at a wave means at least one EF task score is non-missing. The analytic sample includes only children meeting the wave-coverage rule: SA = EF observed at both T1 and T2; AU = EF observed at T1–T3 ("strict") or at T1 and T2 ("t12"). Z-scores are standardized using pooled means and SDs across both cohorts and timepoints (separately per EF measure), preserving between-cohort differences in level on the y-axis."
...
p_mean_trend <- ggplot(mean_df, aes(x = timepoint, y = m, group = 1)) +
  ...
  labs(x = "Timepoint", y = "Mean EF (z-score)") +
  theme_bw(base_size = 11)

ggsave("Figure_3.png", p_mean_trend,
...
summ_delta_rz <- function(dr, dz, rope = 0.05) {
  sr <- summ_vec(dr)
  sz <- summ_vec(dz)
  tibble::tibble(
...
# Exclude from each wave only if scored 0 on Cog Flex
...
A substantial floor effect in Australian T1 cognitive flexibility (32.3% scoring zero; see Appendix @fig-ef-boxplots-by-wave) risks artificial correlations through range restriction and biased partial correlation estimates. We therefore report cross-cohort comparisons both on the full sample and in sensitivity analyses excluding zero-scorers; convergence would suggest floor effects do not distort comparisons, divergence that cohort differences partly reflect measurement artefacts.

Each cell in @tbl-rq3-sensitivity-comparison shows the posterior distribution of a pairwise EF partial correlation difference (AU minus SA). Cohort difference panels (T1, T2) report median Δr [95% CrI] at a single wave; positive values indicate a stronger AU correlation, negative values a stronger SA correlation. The difference-in-differences panel reports (Δr_T2−T1)_AU − (Δr_T2−T1)_SA, with positive values indicating greater developmental growth in AU and negative values indicating greater SA growth. P(|Δr|>0.05) is the proportion of posterior draws exceeding a minimally meaningful threshold; credible intervals excluding zero provide the strongest evidence of a true difference.


**Main Analysis (Full Sample)**: At T1, posterior medians suggested modest AU–SA differences in several edges; however, all 95% credible intervals overlapped zero. At T2, cohort differences were smaller and predominantly positive, but again uncertain. In the difference-in-differences analysis, most edges were centered near zero. The largest deviation was observed for cognitive flexibility–inhibition (median = `r round(rq3_ci("Difference-in-differences (ΔAU − ΔSA)", "ef_cogflex - ef_inhibition", "Main analysis")["m"], 2)`; 95% CrI [`r round(rq3_ci("Difference-in-differences (ΔAU − ΔSA)", "ef_cogflex - ef_inhibition", "Main analysis")["lo"], 2)`, `r round(rq3_ci("Difference-in-differences (ΔAU − ΔSA)", "ef_cogflex - ef_inhibition", "Main analysis")["hi"], 2)`]; P(|Δr|>0.05) = `r round(rq3_rope("Difference-in-differences (ΔAU − ΔSA)", "ef_cogflex - ef_inhibition", "Main analysis"), 2)`), though its credible interval still included zero, indicating substantial uncertainty. Results were substantively unchanged when differences were summarized on the Fisher z (atanh) scale (see Appendix @tbl-rq3-gt-edges-dual).

**Sensitivity Analysis (Excluding Cognitive Flexibility Floor Performers)**: Given the pronounced floor effect in Australian T1 cognitive flexibility (`r excl_n("AU","T1")`/`r orig_n("AU","T1")` excluded [`r excl_pct("AU","T1")`%]; AU T2: `r excl_n("AU","T2")`/`r orig_n("AU","T2")` [`r excl_pct("AU","T2")`%]; we refitted networks after wave-specific exclusion of children scoring zero on cognitive flexibility. Estimates were highly similar in direction and magnitude to the main analysis (see @tbl-rq3-sensitivity-comparison), with credible intervals widening due to reduced sample size. Patterns at T2 and in the difference-in-differences analysis were nearly identical. Together, these results indicate that cross-cohort differences in EF network structure are small and uncertain.

Although formal cross-cohort tests were inconclusive — credible intervals for all AU–SA differences included zero — the largest difference-in-differences contrast was observed for the cognitive flexibilty–inhibition edge (median Δ = `r rq3_ci("Difference-in-differences (ΔAU − ΔSA)", "ef_cogflex - ef_inhibition")["m"]`; 95% CrI [`r rq3_ci("Difference-in-differences (ΔAU − ΔSA)", "ef_cogflex - ef_inhibition")["lo"]`, `r rq3_ci("Difference-in-differences (ΔAU − ΔSA)", "ef_cogflex - ef_inhibition")["hi"]`]; P(|Δr|>0.05) = `r rq3_rope("Difference-in-differences (ΔAU − ΔSA)", "ef_cogflex - ef_inhibition")`). At T1, this edge appeared stronger in SA than AU (median Δ = `r rq3_ci("Cohort difference (AU − SA) at T1", "ef_cogflex - ef_inhibition")["m"]`; 95% CrI [`r rq3_ci("Cohort difference (AU − SA) at T1", "ef_cogflex - ef_inhibition")["lo"]`, `r rq3_ci("Cohort difference (AU − SA) at T1", "ef_cogflex - ef_inhibition")["hi"]`]), whereas at T2 the difference was negligible (median Δ = `r rq3_ci("Cohort difference (AU − SA) at T2", "ef_cogflex - ef_inhibition")["m"]`; 95% CrI [`r rq3_ci("Cohort difference (AU − SA) at T2", "ef_cogflex - ef_inhibition")["lo"]`, `r rq3_ci("Cohort difference (AU − SA) at T2", "ef_cogflex - ef_inhibition")["hi"]`]). The remaining edges showed little evidence of cohort differences at either wave or across the T1–T2 interval. 

Taken together, these results do not provide strong evidence for cohort differences in EF network structure. The inconclusive RQ3 results reflect the inherent imprecision of difference-in-differences estimation with the current sample sizes, rather than strong evidence of cohort equivalence — the descriptive patterns in RQ1, particularly the divergent trajectories of the cognitive flexibility–inhibition edge, are neither contradicted nor confirmed. The direction and magnitude of the DiD estimate for this edge (median Δ = 0.20; P(|Δr|>0.05) = 0.90) warrants replication with larger samples and additional timepoints.
...
rq3_comparison <- rq3_edges_dual_combined %>%
  ...
  mutate(across(-c(panel, term),
                ~ ifelse(is.na(.) | . == "", "\u2014", as.character(.))))

col_keys <- c(
  "panel", "term",
  "Main analysis_estimate_r", "Main analysis_rope_p_r",
  "Sensitivity (no floor)_estimate_r", "Sensitivity (no floor)_rope_p_r"
)

section_starts <- which(!duplicated(rq3_comparison$panel))
divider_rows   <- section_starts[-1] - 1

flextable(rq3_comparison, col_keys = col_keys) %>%
  add_header_row(
    values    = c("", "", "Main Analysis", "Sensitivity (No Floor)"),
  ...
  set_header_labels(
    ...
    `Main analysis_rope_p_r`            = "P(|\u0394r|>0.05)",
    `Sensitivity (no floor)_estimate_r` = "\u0394r [95% CrI]",
    `Sensitivity (no floor)_rope_p_r`   = "P(|\u0394r|>0.05)"
  ) %>%
  merge_v(j = "panel", part = "body") %>%
  valign(j = "panel", valign = "top", part = "body") %>%
  border_remove() %>%
  ...
  set_table_properties(layout = "autofit", width = 1) %>%
  add_footer_lines(
    "Note. CrI = credible interval. Sensitivity analysis uses wave-specific exclusion of children scoring 0 on Cognitive Flexibility (AU T1: 75/232 [32.3%]; AU T2: 42/217 [19.4%]; SA T1: 0/217 [0.0%]; SA T2: 7/189 [3.7%])."
  ) %>%
  italic(i = 1, part = "footer")
```
\clearpage
...
# Remove sa_edu_tp and aus_edu_tp (no longer using)
...
Descriptive statistics for socioeconomic status and home learning environment by cohort and timepoint are presented in @tbl-render-table2. We examined whether these factors moderated EF network structure within each cohort.

```{r}
#| include: false

group_levels <- bind_rows(
  aus_income_tp %>% distinct(country, timepoint),
  ...
  distinct(country, timepoint) %>%
  mutate(
    country   = country_for_table(country),   # <- ADD THIS LINE
    timepoint = factor(timepoint, levels = paste0("T", 1:10)),
    group     = paste(country, timepoint, sep = " — ")
  ) %>%
  arrange(match(country, c("Australia", "South Africa")), timepoint) %>%  # <- fix order string
  pull(group) %>%
  unique()
...
# AUS income (categorical) — education rows removed
ses2_wide <- aus_income_tp %>%
  mutate(
    country = country_for_table(country),
    group   = paste(country, timepoint, sep = " — "),
...
au_cols <- grep("Australia", names(paper_table2), value = TRUE)
sa_cols <- grep("South Africa", names(paper_table2), value = TRUE)
```
...
section_starts <- which(!duplicated(paper_table2$section))
divider_rows   <- section_starts[-1] - 1
...
flextable(tbl2, col_keys = c("section", "row", all_data_cols)) %>%
  add_header_row(
    values    = c("", "", "Australia", "South Africa"),
    colwidths = c(1L, 1L, n_au, n_sa),
    top       = TRUE
  ...
  set_table_properties(layout = "autofit", width = 1) %>%
  add_footer_lines(
    "Note. Summaries restricted to children with any EF data at each wave (partial EF allowed). In Australia, household income and the HLE Index are measured once per child and carried across waves; in South Africa, measures are recorded by wave and summarised only where collected. Australian income bands are policy-defined thresholds aligned with the Child Care Subsidy (CCS): low income (AUD $0\u201345,144; 100% CCS subsidy), middle income (AUD $45,145\u2013156,914; sliding-scale subsidy), and high income (>AUD $156,915; no subsidy eligibility)."
  ) %>%
  italic(i = 1, part = "footer")
```
...
Within-cohort moderation analyses compared EF edge strengths across SES strata within each cohort×wave (see Appendix @tbl-rq4-sample-sizes for sample sizes per stratum). In AU at T2, the cognitive flexibility–working memory edge was stronger in middle- relative to high-income children (Δ = `r rq4_mod_report("income","AU","T2","High - Middle","ef_cogflex - ef_workingmem")`; P(|Δ|>0.10) = `r rq4_mod_meaningful("income","AU","T2","High - Middle","ef_cogflex - ef_workingmem")`). In SA at T2, the inhibition–working memory edge was stronger in high- relative to low-income children (Δ = `r rq4_mod_report("income","SA","T2","High - Low","ef_inhibition - ef_workingmem")`; P(|Δ|>0.05) = `r rq4_mod_meaningful("income","SA","T2","High - Low","ef_inhibition - ef_workingmem")`).

```{r}
#| label: tbl-rq4-moderation
#| tbl-cap: "Within-cohort moderation of EF edges by income and HLE. For income, cells show posterior median [95% CrI] for pairwise group differences. For HLE, cells show High - Low differences. ROPE probability indicates P(|difference| > 0.05)."
#| include: true

rq4_tbl_print <- rq4_table %>%
  mutate(across(c(estimate, rope_p),
                ~ ifelse(is.na(.) | . == "", "\u2014", as.character(.))))

col_keys <- c("moderator", "edge", "country", "timepoint",
              "contrast", "n_a", "n_b", "estimate", "rope_p")

section_starts <- which(!duplicated(rq4_tbl_print$moderator))
divider_rows   <- section_starts[-1] - 1

flextable(rq4_tbl_print, col_keys = col_keys) %>%
  set_header_labels(
    moderator = "",
    edge      = "Edge",
    country   = "Cohort",
  ...
  merge_v(j = "moderator", part = "body") %>%
  valign(j = "moderator", valign = "top", part = "body") %>%
  border_remove() %>%
  hline_top(border = thick, part = "header") %>%
  hline_bottom(border = thin,  part = "header") %>%
  hline(i = divider_rows, border = thin,  part = "body") %>%
  ...
  align(align = "center", part = "header") %>%
  align(align = "center", part = "body") %>%
  align(j = c("moderator", "edge"), align = "left", part = "body") %>%
  italic(j = "moderator", part = "body") %>%
  padding(j = "edge", padding.left = 14, part = "body") %>%
  fontsize(size = 10, part = "all") %>%
  set_table_properties(layout = "autofit", width = 1) %>%
  add_footer_lines(
    "\u00b9 A and B refer to the two groups being compared (e.g., High\u2013Low: A = High, B = Low)."
  ) %>%
  add_footer_lines(
    "Note. CrI = credible interval; HLE = home learning environment. ROPE threshold = 0.05."
  ) %>%
  italic(i = 1:2, part = "footer")
```

Each cell in @tbl-rq4-moderation represents the posterior distribution of a pairwise EF partial correlation difference between two SES groups within a given cohort and wave, using the same conventions as @tbl-rq3-sensitivity-comparison: median Δ [95% CrI] reflects direction and magnitude of the group difference, and P(|Δ|>0.05) reflects the proportion of posterior draws exceeding a minimally meaningful threshold.

Despite limited statistical power given income group sample sizes, particularly at later Australian waves where low-income cells fell below the n = 20 estimation threshold, two SES moderation effects met the meaningfulness criterion. In Australia at T2, high-income children showed a weaker cognitive flexibility–working memory edge than middle-income children (Δ = `r rq4_mod_report("income", "AU", "T2", "High - Middle", "ef_cogflex - ef_workingmem")`; P(|Δ|>0.10) = `r rq4_mod_meaningful("income", "AU", "T2", "High - Middle", "ef_cogflex - ef_workingmem")`). In South Africa at T2, the inhibition–working memory edge was stronger in high- relative to low-income children (Δ = `r rq4_mod_report("income", "SA", "T2", "High - Low", "ef_inhibition - ef_workingmem")`; P(|Δ|>0.05) = `r rq4_mod_meaningful("income", "SA", "T2", "High - Low", "ef_inhibition - ef_workingmem")`). 

\clearpage

# Discussion

The present study applied Bayesian Gaussian Graphical Models to map the conditional architecture of executive function across two longitudinal preschool cohorts from Australia and South Africa. Using age-adjusted partial correlations among inhibitory control, working memory, and cognitive flexibility, we examined how EF components directly relate to one another — moving beyond shared variance to characterize the relational structure of the EF system. Several findings emerge from this analysis. 

First, EF components showed moderate and selective coupling, consistent with partial unity: the inhibition–working memory edge was the most reliable connection across both cohorts and all waves, while cognitive flexibility showed more context-dependent coupling — more strongly linked to inhibition in South Africa and to working memory in Australia. Second, network structure remained stable across adjacent developmental timepoints despite clear increases in EF performance, suggesting a dissociation between the level of executive abilities and the relational architecture that organizes them. Finally, exploratory moderation analyses identified two edges differing across income groups: in Australia at T2, middle-income children showed stronger cognitive flexibility–working memory coupling than high-income children; in South Africa at T2, high-income children showed stronger inhibition–working memory coupling than low-income children — a divergence suggesting that socioeconomic context may modulate EF coupling differently depending on baseline economic conditions, though replication is needed before strong conclusions can be drawn.

## Partial Unity and Context-Dependent Coupling in Early Childhood EF Networks

The inhibition–working memory edge was the most reliable connection across all waves and both cohorts, suggesting these two components share the strongest conditional association once cognitive flexibility and age are accounted for. This converges with @karr2022unity, where the same edge was strongest across ages 3–6, and is consistent with theoretical accounts in which inhibitory control and working memory are closely related facets of a common executive control mechanism [@friedman2017unity; @miyake2000unity].

Within this broadly shared architecture, structure differed between cohorts: in Australia, working memory was the most connected node; in South Africa, inhibition played a comparably central role, with a strong cognitive flexibility–inhibition edge at T1 (`r edge_report("SA", "T1", "Cognitive flexibility–Inhibition")`; `r edge_meaningful("SA", "T1", "Cognitive flexibility–Inhibition")`) that showed some attenuation at T2, though with considerable uncertainty (T2: `r edge_report("SA", "T2", "Cognitive flexibility–Inhibition")`; `r edge_meaningful("SA", "T2", "Cognitive flexibility–Inhibition")`). The cognitive flexibility–working memory edge was weak and uncertain across waves (T1: `r edge_report("SA", "T1", "Cognitive flexibility–Working memory")`; `r edge_meaningful("SA", "T1", "Cognitive flexibility–Working memory")`), and should be interpreted cautiously given the modest posterior evidence. Taken together, these patterns raise the possibility that the relational architecture of executive function may be sensitive to sociocultural and environmental context [@williams2022learning], though replication in independent samples is needed before stronger conclusions can be drawn.

A further difference concerns the cognitive flexibility–inhibition edge, which was weakly supported in the Australian network at T1 (`r edge_meaningful("AU", "T1", "Cognitive flexibility–Inhibition")`) but more credibly present in South Africa (`r edge_meaningful("SA", "T1", "Cognitive flexibility–Inhibition")`). This is theoretically noteworthy: @karr2024network identified the cognitive flexibility–inhibition edge as a marker of EF network maturation, showing the greatest increase with age and stronger weighting in neurotypically developing children relative to those with ADHD across US and Australian samples. The comparatively stronger presence of this edge in South Africa at T1 — among younger children on average — may therefore reflect a different developmental trajectory or timing of EF integration. Caution is warranted however given the pronounced floor effect in Australian cognitive flexibility at T1 (32.5% scoring zero), which may artificially suppress this edge; sensitivity analyses excluding floor performers were substantively unchanged (see RQ3).

We found little evidence of reliable change in global strength or expected influence, which may reflect the brief longitudinal window. Both indices may be comparatively stable over short developmental intervals, such that meaningful change is subtle and difficult to distinguish from measurement uncertainty. This is consistent with prior EF network modelling where global strength and centrality indices within childhood age bands are often estimated with uncertainty, and developmental differences are primarily characterized via between-group contrasts rather than within-person change [@karr2022unity]."

These findings are broadly consistent with developmental factor analytic work: confirmatory factor analyses with preschool samples consistently support unidimensional or two-factor representations of EF, in contrast to the three-factor structure characteristic of adults [@karr2018unity; @lee2013developmental; @wiebe2008using; @willoughby2012measurement]. The network approach complements these findings by revealing which pairwise associations drive partial integration. In the bifactor framework of @friedman2017unity, inhibition loads primarily on a Common EF factor, and the inhibition–working memory coupling observed here may reflect this shared common control variance, even before the three-factor structure has fully differentiated.

## Stability of EF Network Structure Amid Developmental Growth

Our results did not support meaningful change in EF network summaries across waves within either cohort: posterior 95% CrIs for all Δs included 0, and probabilities that edge changes exceeded the ROPE (|Δr| > 0.05) were modest (≈0.6–0.8). Notably, children in both cohorts showed improvements in raw EF scores — consistent with the well-documented rapid growth of executive abilities during the preschool years [@best2010developmental; @diamond2013executive] — yet conditional associations among components remained stable. Network structure and mean performance thus appear to capture dissociable aspects of EF development, consistent with early childhood being a period of relative EF unity [@nelson2016structure; @wiebe2008using] in which components strengthen together rather than becoming more distinct.

Descriptively, working memory was the most centrally connected node in Australia across all three waves, while inhibitory control showed comparably high centrality in South Africa at T1, where the cognitive flexibility–inhibition edge — absent in Australia at T1 — was the strongest in the network. These descriptive differences should be interpreted cautiously: credible intervals for all edge estimates crossed zero, and formal cross-cohort tests (RQ3) were inconclusive, with all AU–SA difference estimates also spanning zero. The descriptive patterns are therefore neither confirmed nor contradicted by the comparative analyses. 

Notwithstanding this uncertainty, one interpretation of the descriptive pattern invokes the role of environmental adversity in shaping executive development. The South African sample was drawn from low-income Cape Town communities characterized by informal housing, poverty, and community violence [@cook2024risk]. Environments characterized by chronic stress and threat can selectively promote the development of reactive, threat-focused cognitive processes — including inhibitory control — potentially at the expense of other executive capacities [@blair2016poverty; @sheridan2014dimensions]. A parallel theoretical and empirical literature points away from deficit accounts of adversity, and to the role of contextual and cultural factors in shaping EF development adaptively [@doebel2020rethinking; @niebaum2025adaptive; @frankenhuis2020hidden], to sometimes result in surprising and high EF performance, despite adversity [@howard2020challenging]. Whether reflecting a stress response or contextual adaptation, the more prominent role of inhibition in the South African network may reflect an environment in which inhibitory control is particularly salient and practiced, resulting in stronger coupling between inhibitory processes and cognitive flexibility in early childhood - though this interpretation is speculative and requires further investigation.

## Socioeconomic Moderation of EF Network Structure

Exploratory moderation analyses identified two edges differing across income groups; in Australia at T2, higher household income was associated with a weaker cognitive flexibility–working memory edge relative to middle-income families — possibly reflecting greater EF differentiation in higher-SES environments, where a weaker edge may indicate reduced reliance on one ability to engage another rather than poorer functioning [@karr2022unity]. However, this is one of several possible accounts, and given the exploratory nature of this analysis and the nascent application of network modelling to EF cognitive data, strong interpretive claims about the meaning of edge strength differences remain premature. 

In South Africa at T2, higher household income was associated with stronger inhibition–working memory coupling. The divergence between cohorts — weaker coupling with higher income in Australia, stronger in South Africa — points to contextual specificity in how income-related resources shape EF coupling. Differentiation frameworks predict weaker edges in higher-resource contexts [@karr2022unity], while resource-dependent co-development accounts predict stronger edges where economic scaffolding supports joint development [@rosen2020cognitive]. Current theory does not generate clear a priori predictions, and post-hoc accounts risk overfitting narrative to noise; these findings are therefore reported descriptively, and replication in larger samples with granular SES measurement is needed before interpretive weight can be placed on the direction of these effects.

Home learning environment moderation effects were modest and did not reliably exceed the ROPE threshold, despite established links between HLE quality and EF development [@melhuish2008effects; @sylva2004effective].One possibility is that HLE influences the level of EF abilities rather than the relational architecture among components: children from enriched home learning environments may perform better on individual EF tasks without showing systematically different coupling among components. Alternatively, the composite HLE scores used here may not capture the specific dimensions of home experience most relevant to EF network structure. Future work employing more detailed and observational HLE measures may be better positioned to detect HLE-related differences in EF network topology. For example, measures may need to move away from frequency-based indices (“how many books can you access?”, “how often do you read to your child?”) to indices that capture the quality of EF supportive interactions between caregivers and children around resources (as suggested by the literature on interaction quality in childcare settings [@howard2020measuring].

## Methodological Contributions and Limitations

The present study has several methodological features worth noting. To our knowledge, this is  the first study to apply BGGM to cross-cultural EF data, extending network approaches to a comparative developmental context. BGGM offer a systems-level complement to the latent variable approaches that have characterized much of the existing literature: whereas factor models treat shared variance as attributable to an unobserved common factor, network models treat the conditional associations among components as the primary target of inference [@borsboom2021network; @cramer2010comorbidity]. By estimating age-adjusted partial correlations, this approach captures direct pairwise coupling after removing shared age-related variance and covariance with other components — providing a less confounded view of specific relationships than zero-order correlations. While interfactor correlations are estimable in standard factor models, the focus of inference typically centres on loadings and model fit rather than on the relative strength of specific pairwise dependencies; network models make those dependencies the primary inferential target [@karr2018unity; @borsboom2021network]. ROPE-based inference offers a further advantage, providing a principled alternative to binary significance testing well-suited to modest sample sizes: rather than asking whether an edge differs from zero, it asks whether the effect is large enough to be practically meaningful.

Our findings are subject to several limitations. First, with only three nodes, graph-theoretic metrics such as clustering and path length cannot be meaningfully interpreted; the value of the network approach here lies in the interpretability of individual edge weights and their comparison across cohorts and timepoints. Second, the EF battery comprised a single task per construct, limiting construct reliability and precluding correction for measurement error; future studies employing multiple tasks per domain would allow latent network models accounting for task impurity. Third, the South African cohort lacked a third wave, limiting developmental inference to a single T1–T2 transition. Fourth, attrition in the Australian sample — particularly the reduction to n = 105 at T3 — increases uncertainty in late-wave estimates and raises the possibility of selective retention.

# Conclusion

This study applied Bayesian Gaussian Graphical Models to map the conditional architecture of executive function across two longitudinal preschool cohorts from Australia and South Africa, extending network approaches to a cross-cultural developmental context. Three findings emerge. First, EF components showed moderate and selective coupling consistent with partial unity: the inhibition–working memory edge was the most reliable connection across both cohorts and all waves, while cognitive flexibility showed more context-dependent coupling — more strongly linked to inhibition in South Africa and to working memory in Australia. Second, network structure remained stable across adjacent developmental timepoints despite clear gains in EF performance, suggesting a dissociation between the level of executive abilities and the relational architecture that organises them. Third, exploratory moderation analyses indicated that socioeconomic context may modulate specific aspects of EF coupling in ways that differ by environmental context, though the divergent direction of effects across cohorts resists simple interpretation and replication in larger samples is needed. Together, these findings advance a network perspective on EF development that is complementary to the latent variable tradition, and highlight the value of examining not just how well children perform on EF tasks, but how the components of executive control are functionally coupled as a system.
...
# Executive Function across waves {.appendix}
```{r}
...
# set up plot scales
.scale_colour_country <- function() scale_colour_discrete()
...
ggsave("Figure_4.png", p_age_by_wave,
       width = 8, height = 7, units = "in", dpi = 300)

knitr::include_graphics("Figure_4.png")
```

# Measurement Invariance Testing {.appendix}
```{r}
...
df_mi <- df_mi_wide %>%
  ...
  mutate(across(c(inh_cat, wm_cat, cf_cat),
...
# Configural
if (file.exists("models/fit_config_b.rds")) {
  fit_config_b <- readRDS("models/fit_config_b.rds")
} else {
  fit_config_b <- bcfa(
    model_ef,
    data        = df_mi,
  ...
  saveRDS(fit_config_b, "models/fit_config_b.rds")
}

# Metric
if (file.exists("models/fit_metric_b.rds")) {
  fit_metric_b <- readRDS("models/fit_metric_b.rds")
} else {
  fit_metric_b <- bcfa(
    model_ef,
    data        = df_mi,
  ...
  saveRDS(fit_metric_b, "models/fit_metric_b.rds")
}

# Scalar
if (file.exists("models/fit_scalar_b.rds")) {
  fit_scalar_b <- readRDS("models/fit_scalar_b.rds")
} else {
  fit_scalar_b <- bcfa(
    model_ef,
    data        = df_mi,
  ...
  saveRDS(fit_scalar_b, "models/fit_scalar_b.rds")
}
```

```{r}
#| label: invariance-compare-bayes
#| include: false

# Compare configural vs metric
cmp_config_metric <- blavCompare(fit_config_b, fit_metric_b)

# Compare metric vs scalar
cmp_metric_scalar <- blavCompare(fit_metric_b, fit_scalar_b)

# Extract LOOIC directly (looic = -2 * elpd_loo)
loo_config <- cmp_config_metric$loo[[1]]
loo_metric  <- cmp_config_metric$loo[[2]]
loo_scalar  <- cmp_metric_scalar$loo[[2]]

# Now extract LOOIC from the loo objects
get_looic    <- function(x) x$estimates["looic", "Estimate"]
get_looic_se <- function(x) x$estimates["looic", "SE"]

invariance_loo <- tibble(
  model    = c("Configural", "Metric", "Scalar"),
  looic    = c(get_looic(loo_config),
               get_looic(loo_metric),
               get_looic(loo_scalar)),
  looic_se = c(get_looic_se(loo_config),
               get_looic_se(loo_metric),
               get_looic_se(loo_scalar))
) |>
  mutate(
    delta_looic = looic - looic[1],
    delta_se    = sqrt(looic_se^2 + looic_se[1]^2)
  )

# Also pull the comparison differences directly from diff_loo
# rows are models, columns include elpd_diff and se_diff
print(cmp_config_metric$diff_loo)
print(cmp_metric_scalar$diff_loo)

```

```{r}
#| label: invariance-check-results
#| include: false

invariance_results <- tibble(
  comparison = c("Configural vs Metric", "Metric vs Scalar"),
  elpd_diff  = c(
    cmp_config_metric$diff_loo["model1", "elpd_diff"],
    cmp_metric_scalar$diff_loo["model1", "elpd_diff"]
  ),
  se_diff    = c(
    cmp_config_metric$diff_loo["model1", "se_diff"],
    cmp_metric_scalar$diff_loo["model1", "se_diff"]
  )
) %>%
  mutate(
    looic_diff = elpd_diff * -2,
    ratio      = abs(elpd_diff / se_diff)
  )
invariance_results

# Both comparisons strongly favor adding constraints -> scalar invariance supported

```

To evaluate cross-cohort comparability of EF indicators, we conducted multi-group Bayesian confirmatory factor analysis (bCFA) across four cohort×wave groups (AU_T1, AU_T2, SA_T1, SA_T2), estimated via Markov chain Monte Carlo sampling in Stan using blavaan (v0.5.10) [@merkle2018blavaan; @merkle2021efficient]. Because cognitive flexibility exhibited pronounced floor effects in Australian children at T1 (32.5% scoring zero), and to accommodate heterogeneous measurement scales across cohorts (cognitive flexibility: 0–12 switch accuracy; working memory: 0–4.33 scaled score; inhibition: 0–1 proportion), we treated all three EF indicators as ordinal variables, segmenting them into tertiles (Low/Medium/High) based on pooled distributions. In ordinal CFA, factor loadings capture the strength of association between the latent EF construct and each indicator, while thresholds capture distributional placement of categories — allowing floor effect severity to be represented through threshold parameters without distorting factor loadings. We fit a one-factor model tested in a sequence of increasingly constrained models (configural, metric, scalar) and compared fit using the leave-one-out information criterion (LOOIC). See Appendix A for further detail.

We estimated a one-factor model in a sequence of increasingly constrained models and compared fit using the leave-one-out information criterion (LOOIC; lower = better predictive accuracy). The configural model showed adequate fit (PPP = 0.391). Constraining factor loadings to be equal across groups (metric invariance) improved predictive fit relative to the configural model (ΔLOOIC = `r round(invariance_results$looic_diff[1], 1)`, SE = `r round(invariance_results$se_diff[1] * 2, 1)`), indicating that the constrained parameterization better captured the data-generating structure. Further constraining thresholds to equality (scalar invariance) improved fit further still (ΔLOOIC = `r round(invariance_results$looic_diff[2], 1)`, SE = `r round(invariance_results$se_diff[2] * 2, 1)`). In both cases, the magnitude of improvement far exceeded its standard error, providing strong evidence that the equality constraints reflect genuine cross-group consistency rather than model misspecification (see @tbl-invariance). Full scalar invariance supports comparison of both network structures and latent performance levels across cohorts [@boer2018detecting].

```{r}
#| label: tbl-invariance
#| tbl-cap: "Bayesian Measurement Invariance: Model Fit and Comparisons"

inv_tbl <- tibble(
  Model       = c("Configural", "Metric", "Scalar"),
  PPP         = c(0.391, NA, NA),
  LOOIC       = c(get_looic(loo_config), get_looic(loo_metric), get_looic(loo_scalar)),
  LOOIC_SE    = c(get_looic_se(loo_config), get_looic_se(loo_metric), get_looic_se(loo_scalar)),
  delta_LOOIC = c(NA, invariance_results$looic_diff[1], invariance_results$looic_diff[2]),
  delta_SE    = c(NA, invariance_results$se_diff[1] * 2, invariance_results$se_diff[2] * 2),
  Decision    = c("Supported", "Supported", "Supported")
) %>%
  mutate(
    PPP         = ifelse(is.na(PPP),         "\u2014", sprintf("%.3f", PPP)),
    LOOIC       = sprintf("%.1f", LOOIC),
    LOOIC_SE    = sprintf("%.1f", LOOIC_SE),
    delta_LOOIC = ifelse(is.na(delta_LOOIC), "\u2014", sprintf("%.1f", delta_LOOIC)),
    delta_SE    = ifelse(is.na(delta_SE),    "\u2014", sprintf("%.1f", delta_SE))
  )
...
flextable(inv_tbl) %>%
  ...
  hline_bottom(border = thick, part = "body") %>%
  # row striping
  bg(i = c(1, 3), bg = "#F5F5F5", part = "body") %>%
  align(align = "center", part = "all") %>%
  align(j = "Model", align = "left", part = "body") %>%
  fontsize(size = 11, part = "all") %>%
  set_table_properties(layout = "autofit", width = 1) %>%
  add_footer_lines(
    "\u00b9 PPP = posterior predictive p-value for configural model; values between 0.05\u20130.95 indicate adequate fit."
  ) %>%
  add_footer_lines(
    "\u00b2 \u0394LOOIC relative to next less-constrained model; negative values indicate the constrained model has better predictive accuracy. Magnitude exceeding 2\u00d7 SE indicates strong preference."
  ) %>%
  add_footer_lines(
    "\u00b3 Bayesian ordinal CFA via Stan (blavaan); four groups: AU_T1, AU_T2, SA_T1, SA_T2."
  ) %>%
  italic(i = 1:3, part = "footer")
```

# Executive Function individual trajectories {.appendix}
```{r}
...
#|   *"EF observed" at a wave means at least one EF task score is non-missing. The analytic sample includes only children meeting the wave-coverage rule: SA = EF observed at both T1 and T2; AU = EF observed at T1–T3 ("strict") or at T1 and T2 ("t12"). Mean ± 95% CI is overlaid. Z-scores use pooled means and SDs across both cohorts and timepoints (separately per EF measure), preserving between-cohort differences in level.
...
p_spaghetti <- ggplot(ef_restricted_lab,
                      aes(x = timepoint, y = value_z, group = id)) +
  ...
  facet_grid(country ~ task) +
  labs(x = "Timepoint", y = "EF (z-score)") +
  theme_bw(base_size = 11)

ggsave("Figure_5.png", p_spaghetti,
       width = 8, height = 5, units = "in", dpi = 300)
...
# Boxplot: Distribution of EF scores across waves {.appendix}
```{r}
#| label: fig-ef-boxplots-by-wave
#| fig-cap: "Executive function scores across waves in Australia and South Africa. Boxplots show the distribution of harmonized task scores at each wave, stratified by country."
#| fig-width: 12
#| fig-height: 5
...
p_box <- ggplot(ef_long_plot, aes(x = wave, y = value, fill = country)) +
  ...
  scale_fill_manual(values = c("AU" = "#4E79A7", "SA" = "#F28E2B")) +
  labs(x = "Wave", y = "Score", fill = "Country") +
  theme_bw(base_size = 11)

ggsave("Figure_6.png", p_box,
       width = 8, height = 5, units = "in", dpi = 300)

knitr::include_graphics("Figure_6.png")
```
\clearpage

# Results reported on the Fisher z scale {.appendix}
```{r}
#| label: tbl-rq3-gt-edges-dual
#| tbl-cap: "Between-cohort differences (edges only) at matched waves (AU − SA at T1/T2) and difference-in-differences for change from T1 to T2 (ΔAU − ΔSA). Columns show posterior median [95% CrI] on the raw partial-correlation scale (Δr) and Fisher-z scale (Δz = atanh(r)). ROPE probabilities are computed on Δr: $P(|\\Delta r| > 0.05)$."
#| include: true

rq3_dual_print <- rq3_edges_dual %>%
  select(-analysis) %>%
  mutate(across(c(estimate_r, rope_p_r, estimate_z),
                ~ ifelse(is.na(.) | . == "", "\u2014", as.character(.))))

col_keys <- c("panel", "term", "estimate_r", "rope_p_r", "estimate_z")

section_starts <- which(!duplicated(rq3_dual_print$panel))
divider_rows   <- section_starts[-1] - 1

flextable(rq3_dual_print, col_keys = col_keys) %>%
  add_header_row(
    values    = c("", "", "\u0394r (raw partial correlation)", "\u0394z (Fisher atanh scale)"),
  ...
  fontsize(size = 10, part = "all") %>%
  set_table_properties(layout = "autofit", width = 1) %>%
  add_footer_lines("Note. CrI = credible interval. ROPE threshold = 0.05.") %>%
  italic(i = 1, part = "footer")
```

# Sample sizes for moderation analysis {.appendix}
```{r}
...
rq4_n_table <- dplyr::bind_rows(
  income_n_summary %>%
    dplyr::transmute(moderator = "Income", country, timepoint,
                     group = income_group, n, ok),
  hle_n_summary %>%
    dplyr::transmute(moderator = "HLE",    country, timepoint,
                     group = hle_group,    n, ok)
) %>%
  dplyr::mutate(
...
col_keys <- c("moderator", "country", "timepoint", "group", "n", "status")

section_starts  <- which(!duplicated(rq4_n_table$moderator))
divider_rows    <- section_starts[-1] - 1
insufficient_rows <- which(rq4_n_table$status == "Insufficient n")
stripe_rows     <- seq(1, nrow(rq4_n_table), by = 2)

flextable(rq4_n_table, col_keys = col_keys) %>%
  set_header_labels(
    moderator = "",
    country   = "Cohort",
    timepoint = "Wave",
  ...
  # highlight insufficient n rows (overrides stripe)
  bg(i = insufficient_rows, bg = "#FFF3E0", part = "body") %>%
  align(align = "center", part = "all") %>%
  align(j = c("moderator", "country", "group"), align = "left", part = "body") %>%
  italic(j = "moderator", part = "body") %>%
  fontsize(size = 11, part = "all") %>%
  set_table_properties(layout = "autofit", width = 0.7) %>%
  add_footer_lines(
    "\u00b9 Minimum n = 20 required for stable network estimation. Groups with insufficient n were excluded from moderation analyses."
  ) %>%
  italic(i = 1, part = "footer")
```

# Sensitivity analysis: Multi-wave restricted sample {.appendix}
```{r}
#| label: sens-multiwave-prep
#| include: false

stopifnot(exists("ef_std"), exists("sa_keep_ids"), exists("au_keep_ids_t12"), exists("nodes"))

# Build T1+T2 restricted sample for both cohorts
keep_ids_t12 <- dplyr::bind_rows(
  sa_keep_ids,
  au_keep_ids_t12
) %>%
  dplyr::mutate(id = as.character(id))

ef_restricted_mw <- ef_std %>%
  dplyr::mutate(
    country   = as.character(country),
    timepoint = as.character(timepoint),
    id        = as.character(id)
  ) %>%
  dplyr::inner_join(keep_ids_t12, by = c("country", "id")) %>%
  dplyr::filter(timepoint %in% c("T1", "T2"))  # explicitly drop T3

ef_net_mw <- ef_restricted_mw %>%
  dplyr::filter(as.character(variable) %in% nodes) %>%
  dplyr::mutate(
    age   = as.numeric(age),
    value = as.numeric(value_z)
  ) %>%
  dplyr::filter(!is.na(age))

# Drop children with all three nodes missing
ef_any_ids_mw <- ef_net_mw %>%
  dplyr::group_by(country, timepoint, id) %>%
  dplyr::summarise(any_node_observed = any(!is.na(value)), .groups = "drop") %>%
  dplyr::filter(any_node_observed) %>%
  dplyr::select(country, timepoint, id)
...
ef_net_mw <- ef_net_mw %>%
  dplyr::inner_join(ef_any_ids_mw, by = c("country", "timepoint", "id"))

# Age-residualize within cohort×wave×node
ef_net_mw <- ef_net_mw %>%
  dplyr::group_by(country, timepoint, variable) %>%
  dplyr::group_modify(~ residise_one(.x)) %>%
  dplyr::ungroup() %>%
...
# Wide format for BGGM
ef_net_wide_mw <- ef_net_mw %>%
  dplyr::select(country, timepoint, id, variable, node) %>%
  tidyr::pivot_wider(names_from = variable, values_from = node) %>%
  dplyr::filter(!dplyr::if_all(dplyr::all_of(nodes), ~ is.na(.)))

# Report sample sizes vs main analysis (T1+T2 only for comparability)
cat("=== MULTI-WAVE SENSITIVITY SAMPLE SIZES (T1+T2 only) ===\n")
dplyr::bind_rows(
  ef_net_wide    %>% dplyr::filter(timepoint %in% c("T1","T2")) %>%
    dplyr::count(country, timepoint) %>% dplyr::mutate(sample = "Main"),
  ef_net_wide_mw %>% dplyr::count(country, timepoint) %>%
    dplyr::mutate(sample = "Multi-wave restricted")
) %>%
  tidyr::pivot_wider(names_from = sample, values_from = n) %>%
  dplyr::mutate(pct_retained = round(100 * `Multi-wave restricted` / Main, 1)) %>%
  print()
```  

```{r}
#| label: sens-multiwave-fit
#| include: false

dat_split_mw <- ef_net_wide_mw %>%
  dplyr::select(country, timepoint, id, dplyr::all_of(nodes)) %>%
  dplyr::group_by(country, timepoint) %>%
  dplyr::group_split()

keys_mw <- ef_net_wide_mw %>%
  dplyr::select(country, timepoint, id, dplyr::all_of(nodes)) %>%
  dplyr::group_by(country, timepoint) %>%
  dplyr::group_keys() %>%
  dplyr::arrange(country, timepoint)

stopifnot(nrow(keys_mw) == length(dat_split_mw))

ggm_fits_mw <- purrr::map(dat_split_mw, fit_one)

fit_tbl_mw <- keys_mw %>%
  dplyr::mutate(res = ggm_fits_mw) %>%
  tidyr::unnest_wider(res)

fit_tbl_uniq_mw <- fit_tbl_mw %>%
  dplyr::filter(!purrr::map_lgl(fit, is.null)) %>%
  dplyr::arrange(country, timepoint) %>%
  dplyr::group_by(country, timepoint) %>%
  dplyr::slice(1) %>%
...
cat("=== MULTI-WAVE SENSITIVITY FITS ===\n")
fit_tbl_uniq_mw %>%
  dplyr::mutate(ok = !purrr::map_lgl(fit, is.null)) %>%
  dplyr::select(country, timepoint, n, ok) %>%
  print()
```

```{r}
#| label: sens-multiwave-rq1-tables
#| include: false

# RQ1 edges — multi-wave restricted (T1+T2 only)
rq1_edges_mw <- fit_tbl_uniq_mw %>%
  dplyr::transmute(country, timepoint, n, fit) %>%
  dplyr::mutate(
    edge_list = purrr::map(fit, ~ edge_draws_list(.x, node_names = nodes)),
    edge_df   = purrr::map(edge_list, ~ tibble::tibble(
      edge = names(.x), draws = unname(.x)))
  ) %>%
  tidyr::unnest(edge_df) %>%
  dplyr::mutate(stats = purrr::map(draws, ~ summ_draws(.x, rope = rope))) %>%
  tidyr::unnest(stats) %>%
  dplyr::transmute(
    country, timepoint, n,
    edge     = vapply(strsplit(edge, " — ", fixed = TRUE),
                      function(z) paste(sort(z), collapse = " - "), character(1)),
    estimate = fmt_ci(q50, q025, q975, digits = 2),
    rope_p   = fmt_p(p_outside_rope, digits = 2),
    analysis = "Multi-wave restricted"
  )

rq1_gs_mw <- fit_tbl_uniq_mw %>%
  dplyr::transmute(country, timepoint, n, fit) %>%
  dplyr::mutate(
    gs_draws = purrr::map(fit, global_strength_draws),
    stats    = purrr::map(gs_draws, ~ {
      qs <- stats::quantile(.x, c(.025, .5, .975), na.rm = TRUE)
      tibble::tibble(q025 = unname(qs[1]), q50 = unname(qs[2]), q975 = unname(qs[3]))
    })
  ) %>%
...
# RQ2 delta edges — T1+T2 only for both cohorts
rq2_pairs_mw <- tibble::tibble(
  country = c("AU", "SA"),
  tp1     = c("T1", "T1"),
  tp2     = c("T2", "T2")
)

rq2_delta_edges_mw <- rq2_pairs_mw %>%
  dplyr::mutate(
    fit1 = purrr::pmap(list(country, tp1), ~ get_fit(..1, ..2, fit_tbl_uniq_mw)),
    fit2 = purrr::pmap(list(country, tp2), ~ get_fit(..1, ..2, fit_tbl_uniq_mw)),
    delta_list = purrr::map2(
      fit1, fit2, ~ edge_delta_draws(.x, .y, node_names = nodes))
  ) %>%
  dplyr::select(country, tp1, tp2, delta_list) %>%
  tidyr::unnest_longer(delta_list, values_to = "draws", indices_to = "edge") %>%
  dplyr::mutate(stats = purrr::map(draws, ~ summ_draws_delta(.x, rope = rope_delta))) %>%
...
# Combine main + sensitivity for T1+T2 only
rq1_compare <- dplyr::bind_rows(
  dplyr::bind_rows(rq1_edges, rq1_gs) %>%
    dplyr::filter(timepoint %in% c("T1", "T2")) %>%
    dplyr::mutate(analysis = "Main (wave-specific)"),
  dplyr::bind_rows(rq1_edges_mw, rq1_gs_mw)
)

rq2_compare <- dplyr::bind_rows(
  rq2_delta_edges %>%
    dplyr::filter(country %in% c("AU","SA"), contrast == "T2 \u2212 T1") %>%
    dplyr::mutate(analysis = "Main (wave-specific)"),
  rq2_delta_edges_mw
)
```
...
```{r}
#| label: tbl-sens-multiwave
#| tbl-cap: "Sensitivity analysis: RQ1 (network structure) and RQ2 (wave-to-wave change) results using multi-wave restricted sample versus main wave-specific sample. T1–T2 contrasts only. Edge estimates are posterior median [95% CrI]; P(|r|>0.05) shown for edges."
#| include: true

# helper to convert raw node keys to readable labels
relabel_edge <- function(edge) {
  dplyr::case_when(
    edge == "Global strength" ~ "Global strength",
    TRUE ~ {
      parts <- strsplit(edge, " - ", fixed = TRUE)[[1]]
      paste(unname(node_to_label[parts[1]]),
            unname(node_to_label[parts[2]]),
            sep = " \u2013 ")
    }
  )
}
...
# RQ1 block
rq1_block <- rq1_compare %>%
  dplyr::mutate(
    panel = "RQ1: Network structure",
    term  = vapply(edge, relabel_edge, character(1))
  ) %>%
  dplyr::select(panel, term, country, timepoint, estimate, rope_p, analysis)

# RQ2 block
rq2_block <- rq2_compare %>%
  dplyr::mutate(
    panel = "RQ2: Wave-to-wave change (T2 \u2212 T1)",
    term  = vapply(term, function(e) {
      e <- gsub(" \u2014 ", " - ", e, fixed = TRUE)
  ...
  dplyr::select(panel, term, country, contrast, estimate, rope_p, analysis) %>%
  dplyr::rename(timepoint = contrast)

sens_long <- dplyr::bind_rows(rq1_block, rq2_block) %>%
  dplyr::mutate(
    cell = dplyr::if_else(
      is.na(rope_p) | rope_p == "\u2014" | rope_p == "",
      estimate,
      paste0(estimate, "; P=", rope_p)
    )
  ) %>%
  dplyr::select(panel, term, country, timepoint, analysis, cell) %>%
  tidyr::pivot_wider(names_from = analysis, values_from = cell) %>%
  dplyr::mutate(
    across(everything(), ~ ifelse(is.na(.), "\u2014", as.character(.)))
  ) %>%
  dplyr::arrange(panel, country, timepoint,
                 dplyr::desc(term == "Global strength"), term)

section_starts <- which(!duplicated(sens_long$panel))
divider_rows   <- section_starts[-1] - 1

flextable(sens_long,
          col_keys = c("panel", "term", "country", "timepoint",
                       "Main (wave-specific)", "Multi-wave restricted")) %>%
  set_header_labels(
    panel                   = "",
    term                    = "Measure",
    country                 = "Cohort",
    timepoint               = "Wave",
    `Main (wave-specific)`  = "Main (wave-specific)",
    `Multi-wave restricted` = "Sensitivity (multi-wave)"
  ) %>%
  merge_v(j = c("panel", "country", "timepoint"), part = "body") %>%
  valign(j = c("panel", "country", "timepoint"), valign = "top", part = "body") %>%
  border_remove() %>%
  hline_top(border = thick, part = "header") %>%
  hline_bottom(border = thin,  part = "header") %>%
  hline(i = divider_rows, border = thin, part = "body") %>%
  ...
  align(align = "center", part = "header") %>%
  align(align = "center", part = "body") %>%
  align(j = c("panel", "term", "country", "timepoint"), align = "left", part = "body") %>%
  italic(j = "panel", part = "body") %>%
  padding(j = "term", padding.left = 14, part = "body") %>%
  fontsize(size = 10, part = "all") %>%
  set_table_properties(layout = "autofit", width = 1) %>%
  add_footer_lines(
    "Note. Multi-wave restricted sample: AU and SA = EF observed at both T1 and T2. CrI = credible interval."
  ) %>%
  italic(i = 1, part = "footer")
```

# Sensitivity analysis: Reseach Question 4 - Attriction across SES variables {.appendix}
Children excluded from RQ4 due to missing moderator data were demographically similar to the included sample in Australia across all waves. In South Africa, excluded children at T2 were slightly younger and more likely to be female than included children (excluded: n = 20; mean age = 4.98 vs 5.39 years; 60.0% vs 43.2% female); findings from the SA moderation analyses should therefore be interpreted with some caution given potential selectivity in moderator data availability.
```{r}
#| label: rq4-attrition-check
#| include: false

# Identify included vs excluded children in RQ4
# Base: ef_net_wide (wave-specific network sample)
# RQ4: additionally requires non-missing income AND hle

rq4_ids_base <- ef_net_wide %>%
  dplyr::distinct(country, timepoint, id)

rq4_ids_included <- ef_net_stratified %>%
  dplyr::distinct(country, timepoint, id) %>%
  dplyr::mutate(rq4_included = TRUE)

rq4_attrition <- rq4_ids_base %>%
  dplyr::left_join(rq4_ids_included, by = c("country", "timepoint", "id")) %>%
  dplyr::mutate(
    rq4_included = tidyr::replace_na(rq4_included, FALSE),
    group = ifelse(rq4_included, "Included", "Excluded")
  )

cat("=== RQ4 ATTRITION BY COHORT×WAVE ===\n")
rq4_attrition %>%
  dplyr::count(country, timepoint, group) %>%
  tidyr::pivot_wider(names_from = group, values_from = n, values_fill = 0) %>%
  dplyr::mutate(pct_included = round(100 * Included / (Included + Excluded), 1)) %>%
  print()

# Join age, sex onto attrition frame
rq4_attrition_demo <- rq4_attrition %>%
  dplyr::left_join(
    age_all_keep %>%
      dplyr::mutate(
        country   = as.character(country),
...
# Summarize age by included/excluded
age_compare <- rq4_attrition_demo %>%
  dplyr::filter(!is.na(age)) %>%
  dplyr::group_by(country, timepoint, group) %>%
  dplyr::summarise(
    n    = dplyr::n(),
  ...
  dplyr::mutate(
    variable = "Age (years)",
    cell     = fmt_mean_sd(mean, sd, digits = 2)
  )

# Summarize sex by included/excluded
sex_compare <- rq4_attrition_demo %>%
  dplyr::filter(!is.na(sex)) %>%
  dplyr::count(country, timepoint, group, sex) %>%
  dplyr::group_by(country, timepoint, group) %>%
  dplyr::mutate(pct = 100 * n / sum(n)) %>%
  dplyr::ungroup() %>%
  dplyr::filter(sex == "Female") %>%
  dplyr::transmute(
    country, timepoint, group,
    variable = "Female, n (%)",
    cell     = paste0(n, " (", sprintf("%.1f", pct), "%)")
  )

# Combine
demo_compare <- dplyr::bind_rows(
  age_compare %>% dplyr::select(country, timepoint, group, variable, cell),
  sex_compare
) %>%
  dplyr::mutate(
    country   = country_for_table(country),
  ...
  dplyr::arrange(country, timepoint, variable)

```

```{r}
#| label: tbl-rq4-attrition
#| tbl-cap: "Demographic comparison of children included versus excluded from RQ4 moderation analyses due to missing income or home learning environment data."
#| include: true

attrition_col_keys <- c("country", "timepoint", "variable", "Included", "Excluded")

section_starts_att <- which(!duplicated(paste(demo_compare$country, demo_compare$timepoint)))
divider_rows_att   <- section_starts_att[-1] - 1

demo_compare %>%
  dplyr::mutate(across(everything(), ~ ifelse(is.na(.), "\u2014", as.character(.)))) %>%
  flextable(col_keys = attrition_col_keys) %>%
  set_header_labels(
    country   = "Cohort",
    timepoint = "Wave",
    variable  = "Measure",
    Included  = "Included in RQ4",
    Excluded  = "Excluded from RQ4"
  ) %>%
  merge_v(j = c("country", "timepoint"), part = "body") %>%
  valign(j = c("country", "timepoint"), valign = "top", part = "body") %>%
  border_remove() %>%
  hline_top(border = thick, part = "header") %>%
  hline_bottom(border = thin,  part = "header") %>%
  hline(i = divider_rows_att, border = thin, part = "body") %>%
  hline_bottom(border = thick, part = "body") %>%
  align(align = "center", part = "header") %>%
  align(align = "center", part = "body") %>%
  align(j = c("country", "timepoint", "variable"), align = "left", part = "body") %>%
  italic(j = "country", part = "body") %>%
  padding(j = "variable", padding.left = 14, part = "body") %>%
  fontsize(size = 10, part = "all") %>%
  set_table_properties(layout = "autofit", width = 1) %>%
  add_footer_lines(
    "Note. Excluded children are those in the wave-specific network sample (RQ1–RQ3) who had missing data on household income or home learning environment at T1. Age reported as mean (SD)."
  ) %>%
  italic(i = 1, part = "footer")
...
# explore: What SES data exists by cohort×wave?
...
# explore: AU income - confirm 3 categories
...
# explore: SA income - check if categorical or continuous
...
# explore 4: Look at actual income variable in raw data
...
# explore: HLE Data Structure and Distribution
