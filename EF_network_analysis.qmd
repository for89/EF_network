...
#| fig-cap: |
#|   **Within-wave EF coupling (posterior partial correlations).** Points show posterior medians; bars show 95% credible intervals. Solid line = 0; dashed lines = ROPE (±0.05).
#| include: true
#| fig.width: 8
#| fig.height: 6
...
paper_table2 %>%
  ...
  tab_source_note(
    source_note = md("*Note.* Summaries restricted to children with any EF data at each wave (partial EF allowed). In Australia, caregiver education, household income, and the HLE Index are measured once per child and carried across waves. In South Africa, measures are recorded by wave and summarised only where collected.")
  )
```
...
### add to global strength discussion
More broadly, the absence of reliable change in both global strength and expected influence may reflect the brief longitudinal window in the present studies. Network-level connectivity (global strength) and node centrality (expected influence) may be comparatively stable over short developmental intervals, such that meaningful change—if present—is subtle and difficult to distinguish from measurement uncertainty. This interpretation is consistent with prior EF network modelling spanning preschool through adulthood, where global strength and centrality indices within childhood age bands are often estimated with considerable uncertainty and developmental differences are primarily characterized via between-group contrasts rather than within-person change [@karr2022unity].
### up until here

The finding that EF components in early childhood are moderately but selectively coupled — rather than either strongly integrated or largely independent — is broadly consistent with a substantial body of developmental factor analytic work. Confirmatory factor analyses with preschool samples consistently support unidimensional or two-factor representations of EF, in contrast to the three-factor unity-diversity structure characteristic of adults [@karr2018unity; @lee2013developmental; @wiebe2008using; @willoughby2012measurement]. The network approach taken here complements these findings by revealing which pairwise associations drive this partial integration: across both cohorts, inhibitory control and working memory shared the strongest conditional association. This is consistent with @karr2022unity who find that the inhibition–working memory edge weight was the strongest at ages 3-6 in a US sample. In the bifactor framework of @friedman2017unity, inhibition loads primarily on a Common EF factor, and the inhibition–working memory coupling observed here may reflect this shared common control variance, even in early childhood when the three-factor structure has not yet fully differentiated.

The network approach offers a complementary perspective to latent variable models by making these pairwise conditional dependencies explicit and estimable. Whereas factor models treat shared variance as attributable to an unobserved common factor, network models treat the associations themselves as the primary target of inference [@borsboom2021network; @cramer2010comorbidity]. Although interfactor correlations are estimable in standard factor models, the focus of inference typically centers on factor loadings and model fit rather than on the relative strength of specific pairwise relationships [@karr2018unity]. 

# this isn't really correct.
This reorientation of focus is consequential: the network approach reveals that cognitive flexibility and inhibition are the least tightly coupled pair of EF components in this age range — consistent with the emergence of shifting as a separable component in later development 
[@lee2013developmental; @menu2024latent].

# come back to this

#old
In a three-node network such as the one examined here, the distinction between these frameworks is most apparent in the edge that is weakest: the cognitive flexibility–inhibition association. In standard factor models, this weak pairwise relationship is subsumed within shared latent variance; in the network, it is directly estimable and reveals that shifting and inhibition are, in fact, the least tightly coupled pair of EF components in this age range — consistent with the emergence of shifting as a separable component in later development [@lee2013developmental; @menu2024latent].
...
The absence of meaningful developmental change in EF network structure across adjacent waves, despite clear increases in mean EF performance across the same period (see @fig-mean-trend below and Appendix @fig-spaghetti-plot), constitutes a substantively important dissociation. Children in both cohorts showed reliable improvement in raw EF scores across waves — consistent with the well-documented rapid growth of executive abilities during the preschool years [@best2010developmental; @diamond2013executive] — yet the conditional associations among EF components remained largely stable in both magnitude and pattern. Network structure and mean performance thus appear to capture dissociable aspects of EF development across this age range, with children improving in raw scores while the relational architecture among inhibitory control, working memory, and cognitive flexibility remained largely unchanged. This pattern is consistent with the early childhood period being one of relative EF unity [@nelson2016structure; @wiebe2008using], in which components strengthen together rather than becoming more distinct.
...
# added from results - march 02
The direction of the Australian moderation effect — where middle-income children showed stronger coupling between cognitive flexibility and working memory than high-income children — may reflect greater differentiation of EF components in higher-SES environments. In EF network development, a weaker edge between two components does not necessarily indicate poorer functioning; it may instead reflect reduced reliance on one ability to engage another, consistent with more specialized and independent EF processes. This interpretation aligns with the broader principle that EF differentiation, rather than integration, may characterize more mature networks, though the application of network modelling to EF cognitive data is sufficiently nascent that strong interpretive claims about the meaning of edge strength differences remain premature. In contrast, the South African result, where higher income predicted stronger inhibition–working memory coupling, is more consistent with resource-dependent co-development of executive control. Across cohorts and waves, HLE moderation effects were modest and did not reliably exceed the ROPE, suggesting that home learning environment exerts limited influence on EF network structure above and beyond income, at least within the range and measurement resolution available here. These results are exploratory and should be interpreted with caution given sample size constraints and multiple testing considerations.
