...
abstract: |
  Executive functions (EFs) are foundational cognitive abilities that develop
...
The brain networks that support EF across diverse task demands have been identified in neuroimaging research with adults. The fronto-parietal network (FPN), comprising dorsolateral prefrontal cortex, inferior and superior parietal lobules, and middle frontal gyrus, and the cingulo-opercular network (CON), including dorsal anterior cingulate cortex and bilateral anterior insula, are the core control networks central to executive processing [@dosenbach2008dual; @niendam2012meta]. The FPN is hypothesized to support adaptive control, while the CON facilitates task-set maintenance and sustained control across trials [@dosenbach2008dual]. A meta-analysis of 193 neuroimaging studies confirmed that a superordinate fronto-cingulo-parietal network is consistently engaged across executive function domains, providing neural evidence for the unity component observed in behavioral models [@niendam2012meta; @engelhardt2019neural]. More recent work has proposed that this shared architecture corresponds to the 'multiple demand' system — a set of frontal, parietal, and posterior temporal regions active across diverse cognitive demands — which may provide the neural substrate for the unity component of executive function [@duncan2024brain].

While shared networks support general executive control, evidence also points to component-specific neural signatures mirroring the diversity observed in behavioral factor structures. Inhibition tasks show strongly right-lateralized activation, particularly in right inferior frontal gyrus and superior parietal cortex [@rodriguez2022inhibition; @zhang2017large]. Working memory tasks engage bilateral dorsolateral prefrontal and posterior parietal cortex, with load-dependent increases in inferior frontal gyrus [@rottschy2012modelling]. Shifting tasks show bilateral activation with engagement of striatal and cerebellar regions [@rodriguez2022inhibition]. High-resolution fMRI work using multimodal imaging has provided fine-grained evidence for this diversity, showing that inhibition, working memory, and shifting tasks produce overlapping activations within common MD territories while each generating unique topographic shifts toward adjacent resting-state networks — stop signal tasks preferentially recruiting CON, rule-switching tasks recruiting the dorsal attention network, and working memory tasks showing reduced default mode deactivation [@assem2024basis]. Executive tasks therefore recruit common control networks while simultaneously engaging component-specific regions [@collette2006exploration; @mckenna2017informing; @assem2024basis], complementing nested-factor behavioral models wherein a Common EF factor coexists with component-specific factors.
...
Recent methodological developments have begun to address limitations inherent in traditional latent variable models. For example, @karr2022unity was among the first to apply network analytic approaches to EF across developmental stages, using Bayesian Gaussian Graphical Models (BGGM) to estimate associations among manifest task variables rather than imposing a latent factor structure. More sophisticated latent network approaches that integrate structural equation modelling with network analysis have further advanced the field by examining how executive constructs relate within a relational framework [@menu2024latent]. These approaches reflect a broader conceptual shift: from viewing cognitive abilities solely as indicators of underlying factors to conceptualizing them as components of dynamically interconnected systems. By modelling conditional associations among executive components, a network perspective provides a systems-level complement to latent approaches and enables more direct investigation of developmental change in executive architecture.
...
Understanding how executive components are organized during early development holds substantial implications for both theory and practice. Network analysis can reveal whether EF components function as tightly coupled systems or operate more independently, informing theoretical models of executive development as well as the design and evaluation of interventions (e.g., @scerif2025enhancing). If components show strong direct coupling, broad interventions targeting general executive control may be most effective; if components are weakly connected, targeted domain-specific training may be more warranted [@li2025structural]. Moreover, longitudinal network analysis offers a means of detecting intervention effects directly — strengthening or reorganization of specific edges following targeted training would provide fine-grained evidence that an intervention has altered the conditional relationships among executive components — though the interpretation of such changes must be grounded in theory, since differentiation accounts would predict that adaptive change involves increasing component independence rather than uniformly stronger coupling.
...
### Australia
The Australian sample was the control group from an intervention study. At baseline there were 232 children randomly assigned to the control group, recruited from 25 preschools across diverse socioeconomic metropolitan and regional areas of New South Wales (the mean of area-level socioeconomic deciles was 5.91 (SD = 2.24, ranging from 1-10). This analytic child sample ranged in age from 3 to 5 years (M = 4.43, SD = 0.38), with a relative balance across sex (46.1% girls). Ninety-four percent of the sample was retained at T2. Limited by available resourcing at T3, a geographically constrained subset of the intervention and control group (i.e., within 1 hour of the lead institution) was pursued, yielding a sample of 105 children. No significant differences were identified between the included and excluded subsets on key demographics and characteristics from T1. For comprehensive details on study sites, recruitment, and sample characteristics, see @howard2020everyday. 
...
### Executive Function
EF was assessed using three measures from the Early Years Toolbox (EYT; @howard2017early), a suite of iPad-based assessments with integrated audio instructions to ensure standardized delivery, sequencing, and timing. The EYT has been validated across diverse cultural contexts, including studies in South Africa where tasks were translated and delivered via the app with built-in audio to accommodate linguistic diversity, ensuring standardized administration across languages without requiring further procedural adjustments  [@cook2019associations; @draper2020understanding], as well as in other countries [@hossain2021international; @munambah202124].

#### Working Memory (Mr. Ant) 
This task requires children to remember the spatial locations of an increasing number of colored stickers placed on a cartoon ant [@howard2017early; @caseintellectual]. Each trial proceeds as follows: Mr. Ant appears with one or more stickers for 5 seconds; a blank screen is presented for 4 seconds; Mr. Ant reappears without stickers and children tap the locations where they believe the stickers were. The task increases in difficulty from one to eight stickers, with three trials at each level. Testing continues until completion or failure on all three trials at the same level. Working memory capacity was indexed by a point score calculated per @howard2017early: one point for each consecutive level with at least two correct trials, plus 1/3 point for each additional correct trial.

#### Inhibition (Go/No-Go) 
This task measures children's ability to inhibit a prepotent response [@howard2017early; @wiebe2012separating]. Children are instructed to tap the screen for "go" trials (catch a fish; 80% of trials) and refrain from tapping for "no-go" trials (avoid sharks; 20% of trials). The task comprises 75 trials divided into three blocks. The preponderance of go trials generates a strong response tendency that children must overcome on no-go trials. Inhibition was indexed by an impulse control score representing the product of go and no-go proportional accuracy, which accounts for both the strength of the prepotent response and children's ability to inhibit it.

#### Cognitive Flexibility (Card Sorting)
This task requires children to flexibly shift between sorting rules [@howard2017early; @zelazo2006dimensional]. Children sort stimuli (e.g., blue rabbits, red boats) first by one dimension (e.g., colour) and then by the alternate dimension (e.g., shape). After completing the pre-switch and post-switch phases, children who meet performance criteria proceed to a border phase in which the required rule depends on whether the stimulus is surrounded by a black border (sort by colour) or not (sort by shape). Cognitive flexibility was scored as switch accuracy: the number of correct responses on the post-switch and border (mixed) phases, excluding the pre-switch phase (range 0–12). We observed a substantial floor effect in the Australian sample, with 75 of 232 children (32.3%) scoring zero at T1. This does not suggest an inability to engage with the task; rather, it indicates that a sizeable proportion of children at this age were unable to execute the rule switch and perseverated on the initial sorting rule, consistent with classic DCCS findings [@zelazo2006dimensional].
...
To distinguish meaningful edges from those that are small or negligible, we adopted a Region of Practical Equivalence (ROPE) approach [@kruschke2018rejecting]. We defined the ROPE as |r| ≤ 0.05, such that edge weights falling within ±0.05 were considered practically equivalent to zero [@huth2025statistical]. For each edge, we computed P(|r| > 0.05): the posterior probability that the edge weight exceeds the ROPE threshold in absolute value. Values close to 1 (e.g., P > 0.90) indicate strong evidence that the edge is meaningfully different from zero, while values close to 0.5 indicate substantial uncertainty and values close to 0 (e.g., P < 0.10) indicate strong evidence for practical equivalence to zero.
...
paper_table1 %>%
  ...
  tab_spanner(label = "Australia",    columns = all_of(au_cols)) %>%
  tab_spanner(label = "South Africa", columns = all_of(sa_cols)) %>%
  cols_label(
    .list = setNames(
      as.list(gsub(".* — ", "", c(au_cols, sa_cols))),
      c(au_cols, sa_cols)
    )
  ) %>%
  tab_options(
  ...
  tab_source_note(
  source_note = md("*Note.* Missing EF data counts reflect missing cells across all three EF measures.")
) %>%
  tab_style(
...
@fig-rq1-combined visualizes the network structure. The spatial layout is held constant across panels to facilitate comparison. As described above, working memory consistently occupied a well-connected position, with edges to both inhibition and cognitive flexibility appearing in all three Australian networks and both South African networks. The cognitive flexibility–inhibition edge was absent in Australia at T1, only emerging at T2 when the relationship strengthened sufficiently, consistent with the pattern of increasing coupling seen in the edge estimates.
...
rq1_table_all %>%
  ...
  tab_source_note(
    source_note = md("*Note.* CrI = credible interval; EI = expected influence; GS = global strength. ROPE threshold = 0.05.") 
  )
```
...
edges_df2 <- edge_tbl %>%
  ...
  dplyr::mutate(
    ...
    facet_combo = factor(facet_combo, levels = c("T1 AU", "T1 SA", "T2 AU", "T2 SA", "T3 AU"))
  )
...
p_forest <- ggplot(edges_df2, aes(x = q50, y = edge_short, colour = edge_short)) +
  ...
  geom_errorbar(aes(xmin = q025, xmax = q975), orientation = "y", width = 0, linewidth = 0.7) +
  geom_point(size = 2.4) +
...
# save network panels to png
net_png <- "fig-rq1-network.png"
png(net_png, width = 700, height = 600, res = 120)
...
forest_png <- "fig-rq1-forest.png"
ggsave(forest_png, p_forest, width = 700/120, height = 600/120, dpi = 120)

# combine into final figure with panel labels
img_net    <- png::readPNG(net_png)
img_forest <- png::readPNG(forest_png)
final_png <- "fig-rq1-combined-final.png" 

png(final_png, width = 1400, height = 640, res = 120)
grid.newpage()

# outer layout: 1 row with 2 columns
pushViewport(viewport(layout = grid.layout(1, 2)))

# left column (A)
pushViewport(viewport(layout.pos.col = 1))
pushViewport(viewport(layout = grid.layout(2, 1, heights = unit(c(0.05, 0.95), "npc"))))

pushViewport(viewport(layout.pos.row = 1))
grid.text("(A)", x = 0.5, y = 0.5, just = "center",
...
pushViewport(viewport(layout.pos.row = 1))
grid.text("(B)", x = 0.5, y = 0.5, just = "center",
          gp = gpar(fontsize = 14, fontface = "bold"))
popViewport()
...
Global strength estimates (sum of absolute partial correlations over unique edges) are reported in @tbl-rq1-gt-all (above) and visualized over time in @fig-rq1-strength-ei (below). Global strength estimates showed substantial overlap in their credible intervals across timepoints, indicating that the data did not provide reliable evidence for change in global strength over time (despite variation in point estimates, for example from T1 to T2 in the SA cohort).

### Centrality (Expected influence)
Expected influence estimates (signed sum of incident partial correlations) are also summarized in @tbl-rq1-gt-all (above) and visualised over time by node in @fig-rq1-strength-ei (below). Across cohorts, expected influence was consistently positive, with working memory tending to show the highest centrality and inhibition also relatively high (particularly from T2 onward in Australia). However, credible intervals overlapped substantially across waves for each node, indicating that apparent shifts in point estimates (e.g., the T1–T2 increase for inhibition in Australia, or the downward shift for inhibition and cognitive flexibility in South Africa) cannot be interpreted as reliable change over time. Overall, the results therefore support a stable relative ordering of node centrality—working memory (and, to a lesser extent, inhibition) contributing most strongly to within-wave coupling—rather than clear evidence of longitudinal change in expected influence.
\clearpage
...
rq2_table_print %>%
  ...
  tab_source_note(
    source_note = md("*Note.* CrI = credible interval; \u0394 = change (later \u2212 earlier wave). ROPE threshold = 0.05.")
  )
```
...
We observed a substantial floor effect in Australian T1 cognitive flexibility (32.3% scoring zero) (see Appendix @fig-ef-boxplots-by-wave), raising potential concerns for cross-cohort network comparisons. Floor effects can induce artificial correlations through range restriction and may bias partial correlation estimates. To address this, we (1) Report cross-cohort comparisons using the full sample, and (2) Conduct sensitivity analyses excluding children who scored zero on cognitive flexibility at each wave. If results converge across analyses, this would suggest floor effects do not meaningfully distort cross-cohort comparisons. Divergence would indicate that apparent cohort differences partly reflect measurement artefacts.

Each cell in @tbl-rq3-sensitivity-comparison represents the posterior distribution of a difference in a pairwise EF partial correlation between the AU and SA cohorts. For the cohort difference panels (T1 and T2), the median Δr [95% CrI] reflects the estimated AU minus SA difference in that edge at a single wave: a positive value indicates AU showed a stronger correlation between those two EF components than SA, while a negative value indicates the reverse. For the difference-in-differences panel, the quantity is (Δr_T2−T1)_AU − (Δr_T2−T1)_SA — that is, how much the AU cohort's change in that edge across waves exceeded SA's change. A positive DiD therefore means the AU cohort showed greater developmental growth in that edge relative to SA, and a negative value means SA grew more. The ROPE-based probability P(|Δr|>0.05) reflects the proportion of posterior draws exceeding a minimally meaningful threshold of 0.05; values approaching 1.0 indicate the posterior is concentrated away from negligible differences. Credible intervals that exclude zero provide the strongest evidence of a true difference, while wide intervals crossing zero indicate that despite non-zero point estimates, substantial uncertainty remains about the direction and magnitude of any cohort difference.
...
rq3_comparison %>%
  ...
  cols_width(
    stub()                              ~ pct(30),
    `Main analysis_estimate_r`          ~ pct(25),
    `Main analysis_rope_p_r`            ~ pct(10),
    `Sensitivity (no floor)_estimate_r` ~ pct(25),
    `Sensitivity (no floor)_rope_p_r`   ~ pct(10)
  ) %>%
  tab_source_note(
    source_note = md("*Note.* CrI = credible interval. Sensitivity analysis uses wave-specific exclusion of children scoring 0 on Cognitive Flexibility (AU T1: 75/232 [32.3%]; AU T2: 42/217 [19.4%]; SA T1: 0/217 [0.0%]; SA T2: 7/189 [3.7%]).")
  )
```
\clearpage
...
rq4_table %>%
  ...
  tab_footnote(
    footnote  = "A and B refer to the two groups being compared (e.g., High\u2013Low: A = High, B = Low).",
    locations = cells_column_labels(columns = n_a)
  ) %>%
  tab_source_note(
    source_note = md("*Note.* CrI = credible interval; HLE = home learning environment. ROPE threshold = 0.05.")
  )
```

Each cell in @tbl-rq4-moderation represents the posterior distribution of the difference in a pairwise EF partial correlation between two SES groups, within a given cohort and wave. The median [95% CrI] quantifies the estimated magnitude and direction of the group difference: a positive value indicates the first-named group (e.g., High income) showed a stronger EF edge than the second (e.g., Low income), while a negative value indicates the reverse. The ROPE-based probability P(|Δ|>0.05) reflects the proportion of posterior draws exceeding a minimally meaningful effect size threshold of 0.05; values approaching 1.0 indicate that the posterior mass is concentrated away from negligible differences. Credible intervals that exclude zero provide the strongest evidence of a difference, while wide intervals crossing zero indicate substantial uncertainty regardless of the point estimate.
...
First, EF components in early childhood show moderate and selective coupling, consistent with partial unity: the inhibition–working memory edge was the most reliable connection across both cohorts and all waves, while cognitive flexibility showed more context-dependent coupling — more strongly linked to inhibition in South Africa and more strongly linked to working memory in Australia (T1 and T2). Second, network structure remained stable across adjacent developmental timepoints despite clear increases in EF performance, suggesting a dissociation between the level of executive abilities and the relational architecture that organizes them. Finally, exploratory moderation analyses identified two edges that differed meaningfully across income groups: in Australia at T2, middle-income children showed stronger cognitive flexibility–working memory coupling than high-income children, while in South Africa at T2, high-income children showed stronger inhibition–working memory coupling than low-income children. The divergent direction of these effects across cohorts suggests that socioeconomic context may modulate specific aspects of EF coupling in ways that differ by baseline economic conditions, though replication in larger samples is needed before strong conclusions can be drawn.

## Partial Unity and Context-Dependent Coupling in Early Childhood EF Networks

These edge-level results point to a consistent pattern across cohorts: EF components in early childhood are moderately but selectively coupled, rather than uniformly interconnected. The inhibition–working memory edge was the most reliable connection across all waves and both cohorts, suggesting that these two components share the strongest conditional association once cognitive flexibility and age are accounted for. This pattern converges with prior network modelling of EF tasks; for example, in @karr2022unity, the partial correlation between inhibition and working memory was likewise the strongest edge in the EF network across ages 3–6, indicating a robust and developmentally stable coupling between these components. Together, these findings are consistent with theoretical accounts in which inhibitory control and working memory are closely related facets of a common executive control mechanism [@friedman2017unity; @miyake2000unity], and extend this evidence into early childhood using a longitudinal, multi-cohort network framework.

Within this broadly shared architecture, the specific topology differed between cohorts — in Australia, working memory emerged as the most connected node, with reliable edges to both inhibition and cognitive flexibility. In South Africa, inhibition played a comparably
connected role, with a strong cognitive flexibility–inhibition edge at T1 (`r edge_report("SA", "T1", "Cognitive flexibility–Inhibition")`; `r edge_meaningful("SA", "T1", "Cognitive flexibility–Inhibition")`) that showed some attenuation at T2, though with considerable uncertainty (T2: `r edge_report("SA", "T2", "Cognitive flexibility–Inhibition")`;
`r edge_meaningful("SA", "T2", "Cognitive flexibility–Inhibition")`). The cognitive
flexibility–working memory edge was weak and uncertain across waves (T1: `r edge_report("SA", "T1", "Cognitive flexibility–Working memory")`; `r edge_meaningful("SA", "T1", "Cognitive flexibility–Working memory")`), and should be interpreted cautiously given the modest posterior evidence. Taken together, these patterns raise the possibility that the relational architecture of executive function may be sensitive to sociocultural and environmental context [@williams2022learning], though replication in independent samples is needed before stronger conclusions can be drawn.

A further difference concerns the cognitive flexibility–inhibition edge, which was 
weakly supported in the Australian network at T1 (`r edge_meaningful("AU", "T1", "Cognitive flexibility–Inhibition")`) but more credibly present in South Africa (`r edge_meaningful("SA", "T1", "Cognitive flexibility–Inhibition")`). This is theoretically noteworthy: @karr2024network identified the cognitive flexibility–inhibition edge as a marker of EF network maturation, showing the greatest increase with age and stronger weighting in neurotypically developing children relative to those with ADHD across US and Australian samples. The comparatively stronger presence of this edge in South Africa at T1 — among younger children on average — may therefore reflect a different developmental trajectory or timing of EF integration, rather than simply a quantitative difference in connectivity. Caution is warranted, however, given the pronounced floor effect in Australian cognitive flexibility scores at T1 (32.5% scoring zero), which may artificially suppress this edge.; sensitivity analyses excluding floor performers are reported in the cross-cohort comparisons section (Research Question 3) and were substantively unchanged.

We found little evidence of reliable change in both global strength and expected influence which may reflect the brief longitudinal window in the present studies. Network-level connectivity (global strength) and node centrality (expected influence) may be comparatively stable over short developmental intervals, such that meaningful change, if present, is subtle and difficult to distinguish from measurement uncertainty. This interpretation is consistent with prior EF network modelling spanning preschool through adulthood, where global strength and centrality indices within childhood age bands are often estimated with uncertainty and developmental differences are primarily characterized via between-group contrasts rather than within-person change [@karr2022unity].

The finding that EF components in early childhood are moderately but selectively coupled — rather than either strongly integrated or largely independent — is broadly consistent with a substantial body of developmental factor analytic work. Confirmatory factor analyses with preschool samples consistently support unidimensional or two-factor representations of EF, in contrast to the three-factor unity-diversity structure characteristic of adults [@karr2018unity; @lee2013developmental; @wiebe2008using; @willoughby2012measurement]. The network approach taken here complements these findings by revealing which pairwise associations drive this partial integration: across both cohorts, working memory and inhibitory control shared the strongest conditional association. This is consistent with @karr2022unity who find that the inhibition–working memory edge weight was the strongest at ages 3-6 in a US sample. In the bifactor framework of @friedman2017unity, inhibition loads primarily on a Common EF factor, and the inhibition–working memory coupling observed here may reflect this shared common control variance, even in early childhood when the three-factor structure has not yet fully differentiated.

## Stability of EF Network Structure Amid Developmental Growth

Our results did not support meaningful change in EF network summaries across waves within either cohort. Posterior 95% CrIs for all Δs included 0, and probabilities that edge changes exceeded the ROPE (|Δr| > 0.05) were modest (≈0.6–0.8), indicating substantial uncertainty and no clear support for meaningful change. Notably, children in both cohorts did show improvements in raw EF scores across time points — consistent with the well-documented rapid growth of executive abilities during the preschool years [@best2010developmental; @diamond2013executive] — yet the conditional associations among EF components remained stable in both magnitude and pattern. Network structure and mean performance thus appear to capture dissociable aspects of EF development: children improved in raw scores while the relational architecture among components stayed largely unchanged, consistent with early childhood being a period of relative EF unity [@nelson2016structure; @wiebe2008using], in which components strengthen together rather than becoming more distinct.

Descriptively, working memory emerged as the most centrally connected node in the Australian cohort, with edges to both inhibition and cognitive flexibility across all three waves, while in the South African cohort inhibitory control showed comparably high centrality at T1 and the cognitive flexibility–inhibition edge — absent in Australia at T1 — was the strongest edge in the T1 network. The two cohorts were similarly connected overall at T1, though the pattern of coupling among components differed between them. These descriptive differences should be interpreted cautiously, however, as credible intervals for all edge estimates crossed zero, precluding firm conclusions about the reliability of any individual edge or cross-cohort difference.

One interpretation of this pattern invokes the role of environmental adversity in shaping executive development. The South African sample was drawn from low-income communities in Cape Town characterized by informal housing, poverty, and exposure to community violence [@cook2024risk]. Environments characterized by chronic stress and threat can selectively promote the development of reactive, threat-focused cognitive processes — including inhibitory control — potentially at the expense of other executive capacities [@blair2016poverty; @sheridan2014dimensions]. A parallel theoretical and empirical literature points away from deficit accounts of adversity, and to the role of contextual and cultural factors in shaping EF development adaptively [@doebel2020rethinking; @niebaum2025adaptive; @frankenhuis2020hidden], to sometimes result in surprising and high EF performance, despite adversity [@howard2020challenging]. Whether viewed as a response to stress, or as adaptation to contextual factors, the more prominent role of inhibition in the South African EF network may reflect an environment in which inhibitory control is particularly salient and practiced, resulting in stronger coupling between inhibitory processes and cognitive flexibility in early childhood. This interpretation is speculative and cross-sectional with respect to the cohort comparison; nevertheless, it suggests that the topology of the EF network may be a more sensitive index of environmental influence than global connectivity measures.

## Socioeconomic Moderation of EF Network Structure

Exploratory moderation analyses identified two edges that differed across income groups, though these should be interpreted cautiously given small income-group sample sizes at later waves and the inflated risk of false positives associated with multiple comparisons across edges. In Australia at T2, higher household income was associated with a weaker cognitive flexibility–working memory edge relative to middle-income families. The direction of this moderation effect may reflect greater differentiation of EF components in higher-SES environments — in EF network development, a weaker edge between two components does not necessarily indicate poorer functioning, but may instead reflect reduced reliance on one ability to engage another, consistent with more specialized and independent EF processes [@karr2022unity]. However, this is one of several possible accounts, and given the exploratory nature of this analysis and the nascent application of network modelling to EF cognitive data, strong interpretive claims about the meaning of edge strength differences remain premature.

In South Africa at T2, higher household income was associated with stronger inhibition–working memory coupling. The directional divergence between Australian and South African income moderation effects — where higher income predicted weaker cognitive flexibility–working memory coupling in Australia but stronger inhibition–working memory coupling in South Africa — is interesting, though difficult to interpret. Both patterns are consistent with plausible theoretical accounts: differentiation frameworks would predict weaker edges in higher-resource contexts as EF components become more specialized and independent [@karr2022unity], while resource-dependent co-development accounts would predict stronger edges where greater economic scaffolding supports the joint development of closely related components [@rosen2020cognitive]. That the findings pattern differently across the two samples may reflect genuine contextual variation in how income-related resources shape EF network organisation — or may simply reflect sampling variability and the instability of moderation estimates in small groups. Current theory does not generate clear a priori predictions about the direction of income moderation effects on specific edges, and post-hoc mechanistic accounts risk overfitting narrative to noise. These findings are therefore reported descriptively, and replication in larger, more representative samples with granular SES measurement is necessary before any interpretive weight can be placed on the direction of these effects.

Home learning environment moderation effects were modest and did not reliably exceed the ROPE threshold. This is somewhat surprising given established links between HLE quality and EF development [@melhuish2008effects; @sylva2004effective]. One possibility is that HLE influences the level of EF abilities rather than the relational architecture among components: children from enriched home learning environments may perform better on individual EF tasks without showing systematically different coupling among components. Alternatively, the composite HLE scores used here may not capture the specific dimensions of home experience most relevant to EF network structure. Future work employing more detailed and observational HLE measures may be better positioned to detect HLE-related differences in EF network topology. For example, measures may need to move away from frequency-based indices (“how many books can you access?”, “how often do you read to your child?”) to indices that capture the quality of EF supportive interactions between caregivers and children around resources (as suggested by the literature on interaction quality in childcare settings [@howard2020measuring].

## Methodological Contributions and Limitations

The present study has several methodological features worth noting. Bayesian Gaussian Graphical Models offer a systems-level complement to the latent variable approaches that have characterized much of the existing literature: whereas factor models treat shared variance as attributable to an unobserved common factor, network models treat the conditional associations among components as the primary target of inference [@borsboom2021network; @cramer2010comorbidity]. By estimating age-adjusted partial correlations, this approach captures direct pairwise coupling after removing shared age-related variance and covariance with other components — providing a less confounded view of specific relationships than zero-order correlations. While interfactor correlations are estimable in standard factor models, the focus of inference typically centres on loadings and model fit rather than on the relative strength of specific pairwise dependencies; network models make those dependencies the primary inferential target [@karr2018unity; @borsboom2021network]. The use of ROPE-based inference offers a further methodological advantage, providing a principled alternative to binary significance testing that is well-suited to the modest sample sizes common in developmental research: rather than asking whether an edge differs from exactly zero, it asks whether the effect is large enough to be practically meaningful.
...
This study mapped the conditional architecture of executive function in early childhood using Bayesian network analysis across two longitudinal cohorts from Australia and South Africa. The results reveal that EF in early childhood is characterised by partial unity: the inhibition–working memory edge was the most reliable connection across both cohorts and timepoints, while cognitive flexibility showed more context-dependent coupling — more strongly linked to inhibition in South Africa and more strongly linked to working memory in Australia. Network structure remained stable across adjacent developmental timepoints despite substantial gains in EF performance, suggesting a dissociation between the level of executive abilities and the architecture that organizes their interrelations. Exploratory moderation analyses further indicated that socioeconomic context may modulate specific aspects of EF coupling: in Australia, middle-income children showed stronger cognitive flexibility–working memory coupling than high-income children, while in South Africa, high-income children showed stronger inhibition–working memory coupling than low-income children — a divergence whose interpretation remains open but which points to the potential role of environmental context in shaping EF network organisation. Together, these findings advance a network perspective on EF development that is both complementary to and distinct from the latent variable tradition, and highlight the value of examining not just how well children perform on EF tasks, but how the components of executive control are functionally coupled as a system.
...
tibble(
  ...
  PPP        = c(0.391, NA, NA),
  LOOIC      = c(
    get_looic(loo_config),
    get_looic(loo_metric),
    get_looic(loo_scalar)
  ),
  LOOIC_SE   = c(
    get_looic_se(loo_config),
    get_looic_se(loo_metric),
    get_looic_se(loo_scalar)
  ),
  delta_LOOIC = c(NA,
    invariance_results$looic_diff[1],
    invariance_results$looic_diff[2]
  ),
  delta_SE   = c(NA,
    invariance_results$se_diff[1] * 2,
    invariance_results$se_diff[2] * 2
  ),
  ...
  fmt_number(columns = PPP, decimals = 3) |>
  fmt_number(columns = c(LOOIC, LOOIC_SE, delta_LOOIC, delta_SE), 
             decimals = 1) |>
  tab_footnote(
    footnote  = "PPP = posterior predictive p-value for configural model; values between 0.05–0.95 indicate adequate fit.",
    locations = cells_column_labels(columns = PPP)
  ) |>
  tab_footnote(
    footnote  = "ΔLOOIC relative to next less-constrained model; negative values indicate the constrained model has better predictive accuracy. Magnitude exceeding 2× SE indicates strong preference.",
    locations = cells_column_labels(columns = delta_LOOIC)
  ) |>
  tab_footnote(
    footnote  = "Bayesian ordinal CFA via Stan (blavaan); four groups: AU_T1, AU_T2, SA_T1, SA_T2.",
...
rq3_edges_dual %>%
  ...
  tab_stubhead(label = "Edge") %>%
  tab_spanner(
    label   = md("\u0394r (raw partial correlation)"),
    columns = c(estimate_r, rope_p_r)
  ) %>%
  tab_spanner(
    label   = md("\u0394z (Fisher atanh scale)"),
    columns = c(estimate_z)
  ) %>%
  tab_style(
    style     = cell_text(align = "center"),
    locations = cells_column_spanners(spanners = "\u0394z (Fisher atanh scale)")
  ) %>%
  cols_label(
  ...
  tab_source_note(
    source_note = md("*Note.* CrI = credible interval. ROPE threshold = 0.05.")
  )
```
