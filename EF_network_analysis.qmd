---
title: "Modeling the Architecture of Executive Function Networks: A Longitudinal Bayesian Network Analysis Across Two Countries" 
...
author-note:
  disclosures:
    conflict-of-interest: "The authors have no conflict of interest to declare. Fionnuala O’Reilly was supported by a doctoral training scholarship from the Economic and Social Research Council, and Gaia Scerif was supported in this work by the Economic and Social Research Council (Grants ES/X013561/1 and ES/W002914/1). *Joint senior authorship: Catherine E. Draper, Steven J. Howard, and Gaia Scerif contributed equally to this work."

abstract: "Executive functions (EFs) are foundational cognitive abilities that develop rapidly during early childhood. Although latent variable approaches have characterized the unity and diversity of EF structure extensively, less is known about the conditional associations among EF components—how these associations change as children develop, and whether they vary across  sociocultural contexts. The present study applied Bayesian Gaussian Graphical Models to model age-adjusted EF networks across two longitudinal preschool cohorts from Australia (N = 232; M age = 4.43 years; three waves) and South Africa (N = 217; M age = 4.70 years; two waves). Results suggest moderate and selective coupling among EF components, consistent with partial unity: the inhibition–working memory edge was the most reliable connection across cohorts and waves. Despite substantial gains in EF performance across waves, network structure remained stable over time, suggesting a dissociation between the level of executive abilities and the relational architecture organizing them. Exploratory moderation analyses identified income-related differences in specific edges at T2 in both cohorts, though these findings require replication in larger samples. Together, these results advance a network perspective on early EF development that complements latent variable approaches, highlighting the value of examining not just children's performance levels but the conditional architecture organizing EF components across development."
...
The brain networks supporting EF have been characterized in neuroimaging research with adults. The fronto-parietal network (FPN) and cingulo-opercular network (CON) are the core control networks central to executive processing [@dosenbach2008dual; @niendam2012meta], with a superordinate fronto-cingulo-parietal network consistently engaged across EF domains — providing neural evidence for the unity component observed in behavioral models [@niendam2012meta; @duncan2024brain]. Alongside this shared architecture, component-specific neural signatures mirror the diversity observed in behavioral factor structures: inhibition, working memory, and shifting tasks produce overlapping activations within common network territories while each recruiting component-specific regions [@assem2024basis; @rodriguez2022inhibition; @rottschy2012modelling], complementing nested-factor behavioral models wherein a Common EF factor coexists with component-specific factors. During early childhood, these networks are less differentiated than in adults, with weaker fronto-parietal connectivity at ages 3–4 strengthening by ages 4–5 [@buss2018changes; @fiske2019neural; @kelsey2025forming]. Across development, activation patterns show progressive specialization within FPN and CON [@crone2006neurocognitive; @rubia2013functional], closely paralleling the behavioral differentiation of EF components through synaptic pruning, myelination, and refinement of functional connectivity [@baum2020development; @tamnes2017development] — underscoring that the unity-diversity structure of EF is grounded in the developing architecture of distributed brain networks [@karr2018unity; @li2025structural].

## Methodological Approaches to Studying Executive Organization

Capturing this developmental trajectory in behavioral data requires analytic approaches that can model the structure of EF — not just its level — and track how that structure changes with age. Latent variable methodologies have been central to advancing the unity/diversity framework: confirmatory factor analysis addresses task impurity by extracting shared variance across tasks, enabling comparison of factor structures across age groups while accounting for measurement error [@karr2018unity; @wiebe2008using]. However, latent modelling relies on specific assumptions about how executive abilities are represented, and the factor structure obtained can vary depending on tasks selected, performance indicators, and analytic specification [@miller2012latent; @vandierendonck2021utility]. While interfactor correlations are estimable in standard factor models, the focus of inference typically centers on loadings and model fit rather than the relative strength of specific pairwise dependencies — a meaningful limitation from a systems perspective.

Network analytic approaches address this directly. By estimating partial correlations among observed task variables, Bayesian Gaussian Graphical Models (BGGM) make the conditional dependencies among EF components the primary inferential target [@karr2022unity; @menu2024latent]. With three nodes — reflecting the standard theoretical parsing of EF into inhibition, working memory, and cognitive flexibility — BGGM yields posterior distributions over each partial correlation, enabling principled probabilistic inference about individual edge strength, developmental change, and cross-context differences with full uncertainty propagation. This is not achievable through piecemeal correlation testing.

## The Present Study 

The present study applies BGGM to examine the organization of EF during early childhood across two longitudinal cohorts from Australia and South Africa, estimating age-adjusted partial correlations among inhibitory control, working memory, and cognitive flexibility. This approach captures direct conditional associations among EF components after accounting for shared relationships and age-related variance, enabling investigation of which components show direct coupling, how these associations change developmentally, and whether network structure varies across sociocultural contexts. We additionally examine whether SES moderates EF network structure within each cohort — specifically, whether edges are systematically stronger or weaker among children from high versus low household income and home learning environment backgrounds — to assess whether the relational architecture of EF is sensitive to proximal environmental variation within as well as between sociocultural contexts.
...
The Australian sample was the control group from an intervention study: 232 children from 25 preschools across diverse socioeconomic areas of New South Wales (M age = 4.43 years, SD = 0.38; 46.1% girls). Ninety-four percent were retained at T2; a geographically constrained subset was followed at T3 (n = 105, no significant demographic differences from those not followed up). The South African sample comprised children from four low-income Cape Town communities characterized by informal housing, poverty, and exposure to violence [@cook2024risk]: 217 at T1 (M age = 4.70 years, SD = 0.60; 48.1% female) and 189 at T2. For full recruitment details see @howard2020everyday and @cook2024risk. The cohorts were not matched by design — differing in recruitment context, SES range, and study type — and cross-cohort comparisons should be interpreted with caution, as observed differences may reflect design-level confounds rather than genuine cross-cultural variation in EF network structure.
...
### Income
Caregivers reported gross annual household income in both cohorts using three bands. In Australia, bands were aligned with Child Care Subsidy eligibility thresholds: low (AUD $0–$45,144), middle (AUD $45,145–$156,914), or high (>AUD $156,915). In South Africa, income was reported using seven ordered categories adapted from the National Income Dynamics Survey [@nids2017], collapsed into low (categories 1–2), middle (3–5), and high (6–7) to facilitate cross-cohort comparability and ensure adequate cell sizes.

### The Home Learning Environment
In both cohorts, caregivers reported the frequency of eight home learning/enrichment activities in the past 7 days, with an HLE composite score calculated by summing responses. In Australia, activities (e.g., reading to the child, visiting the library, teaching sports) were rated on a 5-point scale adapted from the EPPSE study [@melhuish2008effects; @sylva2004effective]. In South Africa, activities (e.g., reading books, counting, singing songs) were rated on a 3-point scale (never, sometimes, many times) using a measure developed for the South African context [@dawes2020early; @melhuish2010impact].
...
Descriptive statistics for socioeconomic status and home learning environment by cohort and timepoint are presented in @tbl-render-table2. We examined whether these factors moderated EF network structure within each cohort. Sensitivity analyses indicated that children excluded due to missing moderator data were broadly similar to the included sample, with the exception of SA at T2 where excluded children were slightly younger and more likely to be female (see Appendix @tbl-rq4-attrition).
...
Although wide credible intervals mean these results cannot be interpreted as strong evidence of stability in the psychometric sense, two patterns speak against a purely imprecision-based account. First, EF performance increased reliably across the same interval in both cohorts — demonstrating that the design was sensitive to developmental change in EF level while conditional associations remained largely invariant. Second, the inhibition–working memory edge was consistently among the strongest connections across all waves and both cohorts — a pattern of selective consistency unlikely to arise from measurement noise alone. Together these observations are more consistent with genuine architectural stability during this developmental window than with undifferentiated imprecision [@menu2024latent].

Descriptively, working memory was the most centrally connected node in Australia across all three waves, while inhibitory control showed comparably high centrality in South Africa at T1, where the cognitive flexibility–inhibition edge — absent in Australia at T1 — was the strongest in the network. These descriptive differences should be interpreted cautiously: the comparative analyses (RQ3) were insufficiently powered to resolve whether they reflect genuine cross-cohort variation — a question that warrants replication with larger samples.

Notwithstanding this uncertainty, one interpretation of the descriptive pattern invokes the role of environmental adversity in shaping executive development. The South African sample was drawn from low-income Cape Town communities characterized by informal housing, poverty, and community violence [@cook2024risk]. Environments characterized by chronic stress and threat can selectively promote the development of reactive, threat-focused cognitive processes — including inhibitory control — potentially at the expense of other executive capacities [@blair2016poverty; @sheridan2014dimensions]. A parallel literature points away from deficit accounts, highlighting how contextual and cultural factors shape EF development adaptively [@doebel2020rethinking; @niebaum2025adaptive; @frankenhuis2020hidden], sometimes resulting in high EF performance despite adversity [@howard2020challenging]. Whether reflecting a stress response or contextual adaptation, the more prominent role of inhibition in the South African network may reflect an environment in which inhibitory control is particularly salient and practiced, resulting in stronger coupling between inhibitory processes and cognitive flexibility in early childhood.
...
The present study is, to our knowledge, the first to apply BGGM to cross-cultural EF data, extending network approaches to a comparative developmental context. While interfactor correlations are estimable in standard factor models, the focus of inference typically centers on loadings and model fit rather than on the relative strength of specific pairwise dependencies; network models make those dependencies the primary inferential target [@karr2018unity; @borsboom2021network; @cramer2010comorbidity] — capturing direct pairwise coupling after removing shared age-related variance and covariance with other components. Beyond this structural contrast, ROPE-based inference provides a principled alternative to binary significance testing well-suited to modest sample sizes: rather than asking whether an edge differs from zero, it asks whether the effect is large enough to be practically meaningful.

Several limitations bear on interpretation, however. First, with only three nodes, graph-theoretic metrics such as clustering and path length cannot be meaningfully interpreted; the value of the network approach here lies in the interpretability of individual edge weights and their comparison across cohorts and timepoints. Second, the EF battery comprised a single task per construct, meaning partial correlations reflect a mixture of construct-level associations and task-specific variance that cannot be separated. This concern is most acute for the stability finding — consistent attenuation across waves cannot be ruled out — and for cognitive flexibility edges, where the Card Sorting task's distinctive procedural demands may independently influence conditional associations.A natural extension would apply BGGM within a latent variable network framework [@menu2024latent], addressing task impurity while preserving the inferential advantages of the network approach. Third, the South African cohort lacked a third wave, limiting developmental inference to a single transition. Fourth, attrition in the Australian sample — particularly the reduction to n = 105 at T3 — increases uncertainty in late-wave estimates and raises the possibility of selective retention. Fifth, the stability finding should be interpreted cautiously — credible intervals spanning zero for change scores reflect imprecision as well as stability, and the present design cannot fully distinguish these accounts. A sensitivity analysis examining the magnitude of change detectable given current sample sizes and edge estimates would strengthen future claims of architectural invariance.

# Conclusion

This study applied Bayesian Gaussian Graphical Models to map the conditional architecture of executive function across two longitudinal preschool cohorts from Australia and South Africa, extending network approaches to a cross-cultural developmental context. Three findings emerge. First, EF components showed moderate and selective coupling consistent with partial unity: the inhibition–working memory edge was the most reliable connection across both cohorts and all waves, while cognitive flexibility showed more context-dependent coupling — more strongly linked to inhibition in South Africa and to working memory in Australia. Second, network structure remained stable across adjacent developmental timepoints despite clear gains in EF performance, suggesting a dissociation between the level of executive abilities and the relational architecture that organizes them. Third, exploratory moderation analyses indicated that socioeconomic context may modulate specific aspects of EF coupling in ways that differ by environmental context, though the divergent direction of effects across cohorts resists simple interpretation and replication in larger samples is needed. Together, these findings advance a network perspective on EF development that is complementary to the latent variable tradition, and highlight the value of examining not just how well children perform on EF tasks, but how the components of executive control are functionally coupled as a system.
...
# Within-cohort moderation of EF edges by income and HLE {.appendix}
```{r}
#| label: tbl-rq4-moderation
#| tbl-cap: "**Within-cohort moderation of EF edges.** For income, cells show posterior median [95% CrI] for pairwise group differences. For HLE, cells show High - Low differences. ROPE probability indicates P(|difference| > 0.05)."
#| include: true
...
# Sensitivity analysis: Research Question 4 - Attrition across SES variables {.appendix}
