...
# Continuous SES + HLE (anchored; SA only variables will be NA for AUS columns)
tp_assets <- summ_cont_tp(sa_ses, "nids_assets_sa", sa_any_ids,
                          "Household assets score (SA; NIDS, 0–26)")

tp_hle1 <- summ_cont_tp(sa_ses, "hla_frequency1", sa_any_ids,
                        "HLE activity frequency (sum)")

tp_hle2 <- summ_cont_tp(sa_ses, "num_caregivers_clean", sa_any_ids,
                        "HLE: unique caregiver types involved")

tp_hle3 <- summ_cont_tp(sa_ses, "books_toys_total1", sa_any_ids,
                        "HLE: books/toys total")

tp_hle4 <- summ_cont_tp(sa_ses, "time_total1", sa_any_ids,
                        "HLE: time total")

tp_cont2 <- bind_rows(tp_assets, tp_hle1, tp_hle2, tp_hle3, tp_hle4) %>%
  arrange(country, timepoint, variable)
...
# reuse same group_levels as Table 1 (recompute safely here)
group_levels <- bind_rows(
  tp_cont2 %>% distinct(country, timepoint),
...
# Continuous block (assets + HLE)
metrics2_long <- bind_rows(
  tp_cont2 %>%
    mutate(group = paste(country, timepoint, sep = " — ")) %>%
    transmute(section = "Assets and home learning environment (continuous)",
...
metrics2_wide <- metrics2_long %>%
  mutate(group = factor(group, levels = group_levels)) %>%
  select(section, row, group, value) %>%
  tidyr::pivot_wider(names_from = group, values_from = value) %>%
  arrange(section, row)

# Categorical block (education + AUS income)
